...
use px_core::shade::{ShadeOptions, shade_mask};
use rayon::prelude::*;
...
fn measure(
    ...
    let occluded_at = |p: px_core::math::IVec2| field.copied(p).flatten() == Some(model.occlusion);
...
pub fn run(seeds: &Path, thresholds: &[f32], passes: &[usize]) -> Result<Vec<Record>> {
    ...
    let real: Vec<Record> = files
        .par_iter()
        .flat_map(|p| {
            let Ok(img) = px_io::png::read_rgba(p) else {
                return Vec::new();
            };
