//! **`px atmos` を実装する前に «効くのか» を測る** (`px-calib atmos`)．
//!
...
//! **効かない** — `px aa` の 81.3% (D83) ・サブピクセルの 53.6% (D124) と
//! 同じ量をここで測る．
//!
//! # 測る規則は 2 つ．対照は «何もしない»
//!
...
//! | `between` | **$c$ と空を結ぶ線の上**にあるものだけを候補にし，狙いに最も近いものを選ぶ ([`px_core::atmos::nearest_toward`]) |
//! | 対照 | **動かさない** (`px anim tween` の «動かさない» と同じ役目) |
//!
//! `between` は `px aa` の [`px_core::aa::nearest_between`] と同じ形の条件だが，
//! **終点がパレットの添字ではなく «宣言された空の色»** なので別に書いてある．
//! **実体は道具の側 (`px_core::atmos`) にあり，ここはそれを呼ぶ** — 測る口が
...
use px_core::atmos::nearest_toward;
use px_core::color::{Oklab, Rgba8, distance_sq, oklab_of};
use px_core::palette::Palette;
use px_core::quantize::oklab_to_rgba;
use std::path::Path;
...
pub const SEGMENT_TOLERANCE: f32 = px_core::atmos::AtmosOptions::DEFAULT_TOLERANCE;
...
fn used_indices(canvas: &px_core::canvas::IndexedCanvas, palette: &Palette) -> Used {
