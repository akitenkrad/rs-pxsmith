...
/// 目録を組み立てる (画像はまだ書かない)．
pub fn build(seed: u64, count: u32) -> Manifest {
    ...
    for id in 0..count {
        ...
        let source = sprite::synthesize(source_seed);
...
pub fn generate(dir: &Path, manifest: &Manifest) -> Result<()> {
    std::fs::create_dir_all(dir.join("images"))
        .with_context(|| format!("{} を作れない", dir.display()))?;

    for item in &manifest.items {
        let source = sprite::synthesize(item.source_seed);
