...
//! 前提にしているが，測っていない．**
//!
//! `px-calib flip` (D89) が測ったのは **`px shade` の出力とその左右反転**だけである．
...
//!
//! 群は 2 つ．**`px shade` の出力** (再導出が素直に効くはずの側) と，
//! **実素材** (手で描かれた側) である．
//!
...
fn agreement(canvas: &IndexedCanvas, palette: &Palette, source: LightSource) -> Option<f32> {
    px_lint::rules::shading_agreement(canvas, palette, source)
}
...
fn declared_light(canvas: &IndexedCanvas, palette: &Palette) -> Option<LightSource> {
    let g = px_lint::rules::mean_lightness_direction(canvas, palette)?;
    // `dir` は光源から面へ向かう向きなので，明度が増す向きの逆になる
    Some(LightSource::Directional {
        dir: px_core::math::Vec2 { x: -g.x, y: -g.y },
    })
}
...
fn measure(s: &Subject<'_>) -> Option<Record> {
    ...
    Some(Record {
        ...
        light_x: px_core::outline::light_direction(source).x,
...
pub fn run(seeds: &Path, steps: u8) -> Result<Vec<Record>> {
    ...
    for path in png_files(seeds)? {
        ...
        for (preset, name) in DIRECTIONAL {
            ...
            // 群 2 — `px shade` の出力．**宣言は導出に使った光源そのもの**であり，
