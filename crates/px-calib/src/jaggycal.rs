...
//!
//! `px smooth` は検出したところの画素を動かす．**動かす前に «良い絵で何件鳴るか» を
...
use px_core::geom::distance::{curvature_field, signed_distance};
use px_core::geom::jaggy::{DEFAULT_MAX_MOVE, analyze_canvas, turn_runs};
use px_core::geom::runs::{run_lengths, run_valleys};
...
pub struct Record {
    ...
    pub chains: usize,
    pub jaggies: usize,
    /// 移動上限 (既定 1 画素) に収まるもの．`px smooth` が実際に動かす件数である．
    pub fixable: usize,
    ...
    /// `px smooth` を実際に掛けた結果 (`--apply` のときだけ)．
...
fn opaque_area(canvas: &px_core::canvas::IndexedCanvas) -> i64 {
    canvas
        .pixels()
        .iter()
...
fn measure(path: &Path, max_move: u32, apply: bool) -> Result<Record> {
    ...
    let applied = if apply {
        ...
        let opts = px_core::smooth::SmoothOptions {
            max_move,
            ..px_core::smooth::SmoothOptions::default()
        };
        let before_area = opaque_area(&canvas);
        let before_blocking = blocking_count(&canvas, &palette);
        let r = px_core::smooth::smooth_canvas(&mut smoothed, &opts);
        // **もう一度掛けて動かないか** (収束しているか)
        let again = px_core::smooth::smooth_canvas(&mut smoothed.clone(), &opts);
        Some(Applied {
            moved: r.moved,
...
/// 既定の移動上限 (`px-core` から引く．数値を書き写さない)．
