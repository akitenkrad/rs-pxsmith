...
//! # なぜ実装より先に測るか
//!
//! D169 は «一定の傾きの直線として説明できる区間» を `px smooth` の対象から外し，
...
//! ここでは**`px smooth` が崩した場所の画素を実際に動かしたか**を数える
...
use px_core::geom::jaggy::turn_runs;
use px_core::geom::runs::{
...
/// いけない (D110) ．`analyze_canvas` が返す [`px_core::geom::Jaggy`] には
