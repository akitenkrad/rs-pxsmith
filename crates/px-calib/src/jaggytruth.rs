...
//! # なぜ実素材だけでは閉じないか
//!
//! `px-calib jaggy` は CC0 の実物 61 枚で **24714 本中 146 件 (0.59%)** と測って
...
use px_core::geom::jaggy::analyze_canvas;
...
impl Summary {
    ...
    /// **`px smooth` が清書を書き換えた画素の合計** — 0 でなければ良い絵を壊している．
...
pub fn run(max_move: u32) -> Summary {
    let clean: Vec<Case> = clean_scenes()
        ...
        .map(|(name, kind, canvas)| {
            ...
            let moved = px_core::smooth::smooth_canvas(&mut smoothed, &opts).moved;
            Case {
                name: name.clone(),
                kind,
                runs: report.runs,
