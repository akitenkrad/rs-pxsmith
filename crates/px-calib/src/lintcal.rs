...
//! 閾値が間違っていると分かる．
//!
//! PNG は `px lint` では格子系のルール (2 ・9) しか通らない．色 ・パレット系の
...
use px_lint::{LintConfig, RULES, Report, rules};
use rayon::prelude::*;
...
fn is_blocking(id: u8) -> bool {
    px_lint::rule(id).is_some_and(|r| matches!(r.severity, px_lint::Severity::Blocking))
}
...
fn lint_one(path: &Path, cfg: &LintConfig) -> Result<Record> {
    ...
    let mut report: Report = px_lint::lint_frame(&frame, cfg);
    // 格子系 (2 ・9) は RGBA を見る
    ...
    RULES
        ...
        .map(|r| {
            ...
            (
                r.id,
                r.name,
                matches!(r.severity, px_lint::Severity::Blocking),
                files,
                total,
            )
        })
