    ...
    match defect {
        ...
        Defect::Pillow => {
            ...
            if palette.len() >= 2 {
                ...
                for &(x, y) in &pixels {
                    mask.set(px_core::math::ivec2(x, y), true);
                }
                let d = px_core::geom::signed_distance(&mask);
                let far = pixels
                    .iter()
                    .filter_map(|&(x, y)| d.copied(px_core::math::ivec2(x, y)))
                    .fold(0.0f32, f32::max)
                    .max(1.0);
                for &(x, y) in &pixels {
                    let at = px_core::math::ivec2(x, y);
        ...
        Defect::MonoShadow => {
            ...
            for &(x, y) in &pixels {
                ...
                let mut moved =
                    px_core::color::Oklab::new(here.l, chroma * hue.cos(), chroma * hue.sin());
                // 彩度が 0 だと色相が無い — 固有色の彩度を最低限持たせる
                if chroma < 0.03 {
                    moved.a = 0.05 * hue.cos();
                    moved.b = 0.05 * hue.sin();
                }
                let mut c2 = px_core::quantize::oklab_to_rgba(moved);
                c2.a = c.a;
                img.set(x, y, c2);
            }
        }
...
///
/// > [!note] **`px aa` を過剰な設定で掛ける形では足りなかった．**
/// > 段の下限 2 ・外郭あり ・4 巡で掛けても，中間色の割合は 3.9 〜 10.2% にしか
/// > ならず，**良い絵の 90% 点 (11.2%) を下回った**．`px aa` は «角» にしか置かない
    ...
    for y in 0..h {
        for x in 0..w {
            ...
            for (dx, dy) in [(1, 0), (-1, 0), (0, 1), (0, -1)] {
                ...
                let d = px_core::color::distance_sq(oklab_of(here), oklab_of(c), 1.0);
                if best
                    .as_ref()
            ...
            let mid = px_core::color::Oklab::new(
                (la.l + lb.l) * 0.5,
                (la.a + lb.a) * 0.5,
                (la.b + lb.b) * 0.5,
            );
            let mut c = px_core::quantize::oklab_to_rgba(mid);
            c.a = here.a;
            out.set(x, y, c);
            touched += 1;
        }
