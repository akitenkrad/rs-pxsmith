...
//! | 口 | 入力 | 窓が見るもの |
//! | --- | --- | --- |
//! | `px conform --window` | **拡大された絵** (格子 $s \geq 2$) | 全体が同じ $s$ か |
//! | `px lint` ルール 9 | **L0 のドット絵** (格子 1) | 一部だけ 2 倍で描かれていないか |
//!
//! D37 は «同一の推定器を共有する» と定めており，共有そのものは正しい．
//! **共有できないのは窓の大きさである** — 前者は $s$ の何倍かを要求し，
//! 後者は 16 〜 48 画素の画布に何枚か入ることを要求する．
//!
//! # 真値のある場面
//!
...
/// (D138 ・D151 ・D161 と 4 度踏んでいる)．`px lint` ・`px conform` に
/// そのまま食わせられる形で数枚出す．
pub fn dump(seeds: &[Seed], dir: &std::path::Path) -> anyhow::Result<Vec<String>> {
    ...
    for s in &scenes {
        ...
        px_io::png::write_rgba(&path, &s.canvas)?;
...
///
/// 合成した場面ではなく，`px lint` が実際に受け取る形で測る．
...
mod tests {
    ...
    fn the_window_floor_is_four_cells_per_side() {
        ...
        for scale in [2u32, 3, 4] {
            ...
            assert_eq!(
                l.min_window,
                Some(scale * px_core::grid::MIN_CELLS_PER_WINDOW),
