...
use px_core::project::ProjectOptions;
use px_core::project::{Facing, Projection, SourcePlane, Step, as_written, matrix, project};
use px_core::resample::{ResampleAlgo, ResampleOptions, affine};
...
fn one(
    ...
    Some(ProjRow {
        ...
        jaggies: analyze_canvas(&out, px_core::geom::jaggy::DEFAULT_MAX_MOVE)
            .jaggies
            .len(),
    })
}
...
pub fn grid_vs_thirty(dir: &Path) -> Result<Vec<(&'static str, usize, f32, usize)>> {
    ...
    for (label, m) in cases {
        ...
        for path in png_files(dir)? {
            ...
            n += 1;
            jag += analyze_canvas(&result, px_core::geom::jaggy::DEFAULT_MAX_MOVE)
                .jaggies
                .len() as f32;
