//! **格子を当てても元絵が戻るのか**を測る．
//!
//! `px conform` は格子に沿ってセルの最頻色へ縮小する ([`downscale_modal`]) ．ところが
...
use px_core::grid::downscale_modal;
use px_core::{RgbaCanvas, ivec2};
use rayon::prelude::*;
...
fn compare(
    ...
    let mut palette: Vec<px_core::Rgba8> = Vec::new();
    ...
    let nearest = |c: px_core::Rgba8| -> px_core::Rgba8 {
        let lab = oklab_of(c);
        *palette
            .iter()
