//! パーツ合成 (`px compose`．設計書 5 章 ・4.2 ・D42)．
//!
//! 設計書が決めているのは «アンカー付きパーツ合成 ・variants 展開 ・`--part-delay`»
//! の 3 行だけである．**置き方は実測で決めた** (`px-calib compose`)．
//!
//! # 画布は広げる — 切らない
//!
...
//! 縁取りは «同じ絵を書き換える» 道具なので，シルエットが動くと `px conform` が
...
//!
//! D92 (`px validate`) と同じで，**根拠は出典であって統計ではない**．
...
/// `compose` の反復で解く (設計書 6.9 «関節運動は `Parts` により `px compose` の
