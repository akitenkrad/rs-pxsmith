...
//! 残らない** (D81) ．`px aa` が既に «パレットの中で 2 色の間にある色» を探す形に
//! 直してあるので (D83) ，**同じ [`nearest_between`] を使う** — 付けた中間色を
//! `px clean --remove-aa` で外せる性質もそのまま引き継ぐ．
//!
...
//!
//! # 実素材で測った結果 (`px-calib subpixel`，CC0 61 枚)
//!
//! パレットの色の組のうち «間の色» があるのは**中央 53.6%** (最小 16.7% ・
//! 最大 98.2%) ．`px aa` の 81.3% (D83) より低いのは，あちらが «実際に隣り合って
...
pub struct SubpixelOptions {
    ...
    /// 中間色とみなす許容．`px aa` と同じ値を使う (往復できる性質のため)．
    pub tolerance: f32,
...
pub struct SubpixelReport {
    ...
    /// > 再利用し，新色を作らない») ．
    /// >
    /// > **端から端まで CLI で通して分かった** — `px shade` の 17 色パレットに
    /// > 掛けると «色が 3 増えた» と警告が出た．単体試験も `px-calib` も
...
///
/// 効く見込みを測るための口．`px aa` の 81.3% (D83) と同じ量である．
