//! GIF の書き出し (設計書 2 章の «px-io — aseprite / PNG / GIF / palette»)．
//!
...
//! `px anim ease` が D40 ・D116 で表示周期を扱ったのと同じ作法で
...
///
/// > [!note] **なぜ左上か．**
/// > 途中で画布が広がる道具がある (`px anim squash` は 32x32 を 42x32 にする) ．
    ...
    for f in frames {
        ...
        for i in 0..f.palette.len() {
            let c = f
                .palette
                .get(i as u8)
                .unwrap_or(px_core::color::Rgba8::TRANSPARENT);
...
mod tests {
    ...
    fn tmp(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("px-gif-{}", std::process::id()));
