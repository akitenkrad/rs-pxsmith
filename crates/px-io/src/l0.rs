...
use px_core::frame::{Depth, FrameKind, Layer, LayerMeta, Surface};
use px_core::math::uvec2;
use px_core::{Frame, Palette};
...
impl L0Document {
    ...
    pub fn color_map(&self, path: &Path) -> Result<BTreeMap<char, ColorKey>> {
        ...
        for (key, value) in &self.palette.map {
            ...
            if c != TRANSPARENT_KEY && !COLOR_KEYS.contains(c) {
                return Err(IoError::Core(px_core::CoreError::InvalidColorKey(c)));
            }
    ...
    pub fn resolve_palette(&self, path: &Path) -> Result<(Palette, Option<u8>)> {
        ...
        let index = palette.push(px_core::Rgba8::TRANSPARENT)?;
...
"#;
    ...
    fn invalid_color_key_is_rejected() {
        ...
        assert!(matches!(
            ...
            IoError::Core(px_core::CoreError::InvalidColorKey('!'))
        ));
    }
    ...
    fn non_indexed_layer_is_blocking() {
        ...
        f.layers.push(Layer::new(
            LayerMeta::named("art"),
            Surface::Rgba(px_core::RgbaCanvas::filled(2, 2, Rgba8::TRANSPARENT)),
        ));
