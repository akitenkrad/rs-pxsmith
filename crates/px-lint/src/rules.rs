...
pub struct LintConfig {
    ...
    /// 縁取り 8 枚である (`px-calib lint --outline-interior`) ．
    ///
    /// | 内側の割合 | 良い絵で鳴る | 負例を捕捉 |
    /// | --- | --- | --- |
    /// | 0.02 | 0 / 64 | 0 / 8 |
    ...
    /// > **`px outline --style black` の出力が自分の検査に落ちる**．D58 の優先順位に
    /// > 従い道具を正としてルールを緩めた．
    ...
    ///
    /// **これが無いと `px aa` の出力が落ちる．** `kenney_tile_0101` は幅 2 画素の
    ...
    /// 色相と*同じ*ときを言う．[`px_core::ramp::MIN_HUE_SEPARATION`] (35 度) は
    ...
    pub light: Option<px_core::ramp::LightSource>,
    /// ルール 7 — 明度勾配と光源方向の一致度がこれを下回れば違反とする．
    ///
    /// **`px shade` の出力とその左右反転で決めた** (`px-calib flip`) ．光源が
    ...
    ///
    /// **`px smooth` と同じ値を使う** — 報告の «直せる» が実際に直せることと一致
    /// しなければ意味が無い．数値は書き写さず [`px_core::geom::jaggy::DEFAULT_MAX_MOVE`]
    /// から引く．
    ...
    /// `.hex` にも欄が無いので，`px shade` → `px lint` を CLI で繋いだ時点で消える．
    ...
    /// | **`px shade` の 1 画素ハイライト** | 8 | 0.102 | 0.176 | **0.178** |
    ///
    ...
    /// ルール 13 — 距離場と明度の相関の上限．
    ///
    /// `px-calib pillow` で 3 群を測って決めた (D77) ．**正例が先** (D70) ．
    ///
    /// | 閾値 | 正例で誤爆 (良い絵 61) | 負例を捕捉 (pillow 6) | **`px shade` が鳴る (320)** |
    /// | --- | --- | --- | --- |
    /// | 0.60 | 8.2% | 100% | **0%** |
    /// | 0.75 | 4.9% | 100% | **0%** |
    /// | **0.85** | **1.6%** | **100%** | **0%** |
    ...
    ///
    /// > [!note] **`px shade` の出力は 320 通りすべてで負の相関だった** (最大 $-0.020$) ．
    ...
    ///
    /// 中間色の数え方は [`px_core::aa::intermediate_pixels`] である．**3 群で測った**
    /// (`px-calib aa`) — 正例が先 (D70) ，そして**自作の出力が自作の検査に落ちない**
    /// こと (D58 ・D77 と同じ作法) ．
    ///
    /// | 群 | 中央 | 90% | 95% | 最大 |
    /// | --- | --- | --- | --- | --- |
    /// | 良い絵 (CC0 61 枚) | 0.086 | 0.273 | 0.337 | 0.593 |
    /// | `px aa` を掛けた後 (同 61 枚) | 0.106 | 0.287 | 0.348 | 0.606 |
    /// | **`px shade` の出力 (64 枚 x 5 プリセット)** | 0.210 | 0.383 | 0.399 | **0.527** |
    ...
    /// > `px shade` の出力が鳴った** (端から端まで CLI で通して 38.8%) ．
    /// > D58 の優先順位に従い，**`px shade` の最大 (0.527) の上**に置く．
    ///
    /// 0.55 での成績は **良い絵 1 / 64 ・`px shade` 0 / 320 ・負例 1 / 8** である．
    ///
    /// > [!note] **群は元から分かれていない．** 特徴量を 4 通り試した．
    /// >
    /// > | 試した特徴量 | 結果 |
    /// > | --- | --- |
    /// > | `px clean --remove-aa` が外す画素 | **AA が多いほど減る** (16 画素の上限で枠から外れる) |
    /// > | [`px_core::aa::strip_aa`] の画素マスク | 良い絵で中央 0.175 ・最大 0.670 と緩すぎる |
    /// > | 中点にあるだけ («より広く使われている» を外す) | 良い絵で中央 0.488 ・最大 0.987 |
    /// > | **中点にあり両端がより広い** (採用) | 上の表 |
    /// >
    /// > 負例も «`px aa` を過剰な設定で 4 巡» では 0.039 〜 0.102 にしかならず，
    /// > 良い絵を下回った (`px aa` は角にしか置かないので縁が埋まらない) ．
    ...
    ///
    /// **`px clean --remove-aa` と同じ値を使う** — 数える側と外す側で «中間色» の
...
impl Default for LintConfig {
    fn default() -> Self {
        Self {
            ...
            band_max_gap: 3,
            band_min_runs: 8,
            jaggy_max_move: px_core::geom::jaggy::DEFAULT_MAX_MOVE,
            dither_clump: 6,
            ...
            intermediate_tolerance: px_core::clean::AaOptions::default().tolerance,
...
/// 気にしない — これが無いと `px anim` の出力が自らの lint に大量に落ちる．
...
/// > `px shade` の出力 320 通りに掛けたら 8 件が blocking になった．中身を見ると
...
fn is_a_shading_step(palette: &Palette, a: u8, b: u8, cfg: &LintConfig) -> bool {
    ...
    px_core::color::distance_sq(x, y, 1.0).sqrt() < cfg.stray_min_distance
}
...
fn rule_4_outline_corners(canvas: &IndexedCanvas, cfg: &LintConfig, report: &mut Report) {
    ...
    // なる — これは `px outline` の正しい出力であり (D84 の «内側に描く») ，
    ...
    // 積み重なって $2 \times 2$ になる — `px outline --style black` の出力に実際に
...
/// [`px_core::ramp::MIN_HUE_SEPARATION`] 未満なら単色影である．
///
...
///
/// 閾値を測るための口でもある (`px-calib flip`) ．
pub fn shading_agreement(
    canvas: &IndexedCanvas,
    palette: &Palette,
    source: px_core::ramp::LightSource,
) -> Option<f32> {
...
fn shading_agreement_with(
    ...
    // 実測でも `px shade --preset night` (点光源) は 64 枚すべてで一致度が
    // $-0.54$ 〜 $-1.0$ になり，平行光源の 4 プリセット (最小 $0.714$) と
    // まったく別の分布になった．
    let px_core::ramp::LightSource::Directional { .. } = source else {
        return None;
    };
    let l = px_core::outline::light_direction(source);
...
///
/// 検出そのものは [`px_core::geom::jaggy`] が持っている (M1a で作った) ．ここは
...
fn rule_8_jaggies(canvas: &IndexedCanvas, cfg: &LintConfig, report: &mut Report) {
    ...
    let found = px_core::geom::jaggy::analyze_canvas(canvas, cfg.jaggy_max_move);
...
fn rule_12_banding(canvas: &IndexedCanvas, cfg: &LintConfig, report: &mut Report) {
    use px_core::geom::runs::run_lengths;
    use px_core::geom::{split_monotone, trace_contours};
...
/// > 正しく陰影付けされた凸形状でも相当に相関する — `px shade` の出力自身が
/// > そうである．**`px shade` を正として閾値を決める**．
...
fn silhouette_of(canvas: &IndexedCanvas, palette: &Palette) -> px_core::geom::Mask {
    let mut m = px_core::geom::Mask::new(canvas.width(), canvas.height());
    for p in canvas.bounds().iter() {
...
///
/// 中間色の数え方は [`px_core::aa::intermediate_pixels`] にまとめてある．
...
fn rule_14_too_much_aa(
    ...
    let count = px_core::aa::intermediate_pixels(canvas, palette, cfg.intermediate_tolerance);
...
mod tests {
    ...
    ///
    /// `px shade` は光へ正対した画素に光ランプの最上段を置く．それが平らな面の中に
    /// 1 つだけ落ちると «その色は他に無く単色に囲まれている» を満たしてしまう —
    /// 実際に `px shade` の出力 320 通りで 8 件が blocking になっていた．
    ///
    ...
    ///
    /// ランプの宣言は `.aseprite` にも `.hex` にも欄が無いので，`px shade` の出力を
    /// ファイル経由で `px lint` へ渡すと消える．**色差なら残る．**
    #[test]
    ...
    ///
    /// これが無いと `px anim` の中割りが自らの lint に大量に落ちる．
    #[test]
    fn keyframe_rules_do_not_apply_to_inbetween_frames() {
        ...
        let mut frame = Frame::new(px_core::uvec2(12, 8), palette);
        frame.layers.push(px_core::Layer::new(
            px_core::LayerMeta::named("art"),
            Surface::Indexed(canvas),
        ));

        frame.kind = px_core::frame::FrameKind::Key;
        assert!(has(&lint_frame(&frame, &LintConfig::default()), 8));

        frame.kind = px_core::frame::FrameKind::Inbetween;
        let report = lint_frame(&frame, &LintConfig::default());
    ...
    /// **色相をずらした影は鳴らない** (`px palette ramp` が作る形)．
    #[test]
    ...
    fn from_upper_left() -> px_core::ramp::LightSource {
        px_core::ramp::LightSource::Directional {
            dir: px_core::math::Vec2 { x: 1.0, y: 1.0 },
        }
    }
    ...
    fn rule_14_flags_art_that_is_mostly_intermediate_colours() {
        ...
        for k in 0..5 {
            ...
            colours.push(px_core::quantize::oklab_to_rgba(Oklab::new(
                mid.l + d,
                mid.a + d * 0.2,
                mid.b - d * 0.2,
            )));
