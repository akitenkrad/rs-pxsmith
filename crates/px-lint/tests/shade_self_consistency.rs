//! **自己整合性 — `px shade` の出力は `px lint` を通らなければならない**
...
use px_core::math::{ivec2, vec2};
use px_core::palette::{ChromaCurve, Palette};
use px_core::ramp::{LightPreset, LightSource, build_lighting};
use px_core::shade::{ShadeOptions, shade_to_canvas};
use px_lint::Severity;
...
/// 添字をパレットで解決する (`px lint` が PNG を読むときと同じ形にするため)．
///
/// `px-io` は `px-lint` の依存ではないので `png::resolve` は使えない．
...
/// 1 件ぶんの陰影付けと検査．`px shade` → `px lint` と同じ経路を通す．
fn lint_shaded(
    mask: &Mask,
    preset: LightPreset,
    source: LightSource,
    ...
    report.extend(px_lint::rules::lint_grid(&resolve(&canvas, &palette), &cfg));
    report.sorted()
}

fn describe(report: &px_lint::Report) -> String {
    report
        .violations
        .iter()
        .map(|v| {
...
/// **`px shade` の出力に blocking 違反が 1 件も出ない．**
///
...
/// **`px shade` の出力がルール 13 (pillow shading) に鳴らない．**
///
/// 設計書 6.2 が名指しした自己整合性の相手である．advisory なので blocking の試験では
/// 捕まらず，ここで別に見る．
///
/// 相関の実測は `px-calib pillow` にある — 良い絵 61 枚 ・負例 6 枚 ・
/// **`px shade` の出力 320 通り**を測り，`px shade` は全件が**負の相関** (最大
...
fn shading_is_never_mistaken_for_pillow_shading() {
    ...
    for (name, mask) in shapes() {
        for preset in LightPreset::ALL {
            ...
            .expect("陰影を付けられない");
            let Some(rho) = px_lint::rules::pillow_correlation(&canvas, &palette) else {
                continue;
            };
