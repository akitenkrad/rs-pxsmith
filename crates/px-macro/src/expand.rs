//! `pixels!` の展開．
//!
//! 2 つの書き方をどちらも [`px_io::l0::L0Document`] へ寄せてから
...
use px_io::l0::{L0Document, L0Frame, L0Meta, L0PaletteSpec, RawColorKey};
use px_io::px_core::frame::FrameKind;
use px_io::px_core::{Frame, Rgba8};
...
pub fn expand(input: &Input) -> syn::Result<TokenStream> {
    match input {
        ...
        Input::Inline(inline) => {
            ...
            let doc = L0Document {
                meta: L0Meta {
                    format: px_io::l0::FORMAT_VERSION,
                    name: name.clone(),
...
fn row_span(inline: &crate::input::Inline, e: &px_io::IoError) -> Option<proc_macro2::Span> {
    let px_io::IoError::Parse { line, .. } = e else {
        return None;
    };
    if *line == 0 {
        return None;
...
fn emit(frames: &[Frame], deps: &[PathBuf]) -> TokenStream {
    ...
    let entries = palette.iter().map(|c: &Rgba8| {
        ...
        quote! { ::px_core::Rgba8::new(#r, #g, #b, #a) }
    });
    ...
    let bodies = frames.iter().map(|f| {
        ...
        let kind = match f.kind {
            FrameKind::Key => quote! { ::px_core::FrameKind::Key },
            FrameKind::Breakdown => quote! { ::px_core::FrameKind::Breakdown },
            FrameKind::Inbetween => quote! { ::px_core::FrameKind::Inbetween },
        };
        let layer_name = layer.meta.name.clone();
        let subpixel_exclude = layer.meta.subpixel_exclude;

        quote! {{
            let __canvas = ::px_core::IndexedCanvas::from_pixels(#w, #h, (#bytes as &[u8]).to_vec())
                .expect("pixels!: 画素数は展開時に検証済み")
                .with_transparent(#transparent);
            let mut __meta = ::px_core::LayerMeta::named(#layer_name);
            __meta.subpixel_exclude = #subpixel_exclude;
            let mut __frame = ::px_core::Frame::new(::px_core::uvec2(#w, #h), __palette.clone());
            __frame.duration_ms = #duration;
            __frame.kind = #kind;
            __frame.layers.push(::px_core::Layer::new(
                __meta,
                ::px_core::Surface::Indexed(__canvas),
            ));
    ...
    quote! {{
        #(#includes)*
        let __palette = ::px_core::Palette::new(::std::vec![#(#entries),*])
            .expect("pixels!: パレットは展開時に検証済み");
        let mut __frames: ::std::vec::Vec<::px_core::Frame> =
