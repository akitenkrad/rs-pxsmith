...
use px_core::deform::{SquashAnchor, SquashOptions, VolumeRule, squash};
...
use px_core::subpixel::{SubpixelMethod, SubpixelOptions, SubpixelScope};
use px_core::tween::{ExtrapolateKind, TweenAlign, TweenOptions, extrapolate_mask, tween_series};
...
fn squash_cmd(args: &SquashArgs) -> Result<()> {
    ...
    for (frame, layers) in frames.iter_mut().zip(&placed) {
        ...
        frame.size = px_core::math::uvec2(cw, ch);
    }
    ...
    if (cw, ch) != before {
        ...
        println!(
            "  ** 画布の寸法が変わった ** — 出力先の «寸法はタイルの倍数» 制約を\n\
             壊すことがある．px validate --target で確かめること"
        );
    }
    // **書いていないものを黙らない** (D92 の作法)
    println!(
...
pub struct SubpixelArgs {
    ...
    #[arg(long, default_value_t = px_core::subpixel::DEFAULT_FRACTION)]
    pub fraction: f32,
    ...
    #[arg(long, default_value_t = px_core::subpixel::DEFAULT_MIN_RUN)]
    pub min_run: u32,
}
...
fn subpixel_cmd(args: &SubpixelArgs) -> Result<()> {
    ...
    for frame in &mut frames {
        ...
        for layer in &mut frame.layers {
            ...
            let (out, r) = px_core::subpixel::subpixel(
                canvas,
                &palette,
                &SubpixelOptions {
                    ...
                    scope,
                    exclude: None,
                    tolerance: px_core::subpixel::DEFAULT_TOLERANCE,
...
pub struct AfterimageArgs {
    ...
    pub ramp: Vec<u8>,
    /// 何コマ前まで残すか
    #[arg(long, default_value_t = px_core::afterimage::DEFAULT_TRAIL)]
    pub trail: u32,
    /// 1 コマ古くなるごとにランプを何段落とすか
    #[arg(long, default_value_t = px_core::afterimage::DEFAULT_STEP)]
    pub step: u32,
