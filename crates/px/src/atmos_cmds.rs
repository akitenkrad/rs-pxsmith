//! `px atmos` — 空気遠近法と多重スクロールメタ (設計書 4.4 ・5 章)．
//!
//! **奥行きを持てるのは L0 (`.px.toml`) だけである** — `.aseprite` には対応する
//! 概念が無く (`px-io` の `project_meta`) ，L0 は 1 ファイル 1 レイヤ (D9) なので
...
//! 黙って既定へ倒さず落とす．
//!
//! 判断の根拠は [`px_core::atmos`] のモジュール文書にまとめてある．
...
use clap::Args;
use px_core::atmos::{AtmosOptions, HazeTable, ScrollDoc, ScrollLayer, atmos};
use px_core::color::Rgba8;
use px_core::frame::Depth;
use px_core::ramp::LightPreset;
...
pub fn atmos_cmd(args: &AtmosArgs) -> Result<()> {
    ...
    for input in &args.inputs {
        ...
        let path = PathBuf::from(px_core::compose::expand_template(&args.output, &vars)?);
        ...
        if report.collapsed * 2 >= report.colors && report.colors > 0 {
            println!(
                "    ** 色の半分以上が潰れた ** — そのパレットにその霞は無いという結果で\n\
                 ...
                 \u{3000}\u{3000}ディザの 2 色が 1 色へ潰れると縞が塊になるので，px lint の\n\
