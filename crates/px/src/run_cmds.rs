...
use px_recipe::cache::Cache;
use px_recipe::error::RecipeError;
use px_recipe::expr::{Env, Predicates};
use px_recipe::graph::Graph;
use px_recipe::key::{StepInputs, StepKey, Versions, step_key};
use px_recipe::recipe::{RECIPE_FORMAT, Recipe, ResolvedStep};
use px_recipe::value::Value;
...
impl Predicates for Deferred {
    fn eval(&self, _name: &str, _args: &[Value]) -> px_recipe::Result<Value> {
...
impl Predicates for Live<'_> {
    fn eval(&self, name: &str, args: &[Value]) -> px_recipe::Result<Value> {
        let want = match name {
            "color_count" | "size" => 3,
            _ => 1,
        };
...
impl Live<'_> {
    fn resolve(&self, name: &str, target: &Target) -> px_recipe::Result<PathBuf> {
        match target {
            Target::Input => {
                self.inputs
                    .first()
...
fn compare(op: &Value, left: i64, right: &Value, name: &str) -> px_recipe::Result<Value> {
...
fn load_indexed(
    ...
    name: &str,
    shown: &Path,
) -> px_recipe::Result<(px_core::canvas::IndexedCanvas, px_core::palette::Palette)> {
...
fn has_dither(path: &Path, name: &str, shown: &Path) -> px_recipe::Result<bool> {
    let (canvas, _) = load_indexed(path, name, shown)?;
    let opts = px_core::clean::DenoiseOptions::default();
    Ok(!px_core::clean::detect_dither_noise(&canvas, &opts).is_empty())
}

fn has_transparency(path: &Path, name: &str, shown: &Path) -> px_recipe::Result<bool> {
    let (canvas, _) = load_indexed(path, name, shown)?;
...
fn color_count(path: &Path, name: &str, shown: &Path) -> px_recipe::Result<usize> {
    let (canvas, _) = load_indexed(path, name, shown)?;
...
fn size_of(path: &Path, name: &str, shown: &Path) -> px_recipe::Result<(u32, u32)> {
    let (canvas, _) = load_indexed(path, name, shown)?;
    Ok((canvas.width(), canvas.height()))
}

fn lint_clean(path: &Path, name: &str, shown: &Path) -> px_recipe::Result<bool> {
...
fn build_argv(op: &str, fields: &BTreeMap<String, Value>) -> Result<Vec<String>> {
    ...
    for (key, value) in fields {
        ...
        // > `px compose` の `--part` は id が `parts` である (`Vec` の欄名) ．
...
/// > `px shade out.px.toml` は隣に `out.px.hex` も書く (D127) ．レシピが
/// > `output` に書いていなければ，キャッシュから戻すのは `.px.toml` だけになり，
/// > **`px run` は «3 件当たり» と言って 0 で終わるのに，出力が読めない**．
/// > 端から端まで通して見つけた．
/// >
/// > 直し方を 2 つに分ける．
/// >
...
pub fn run(args: &RunArgs) -> Result<()> {
    ...
    let mut progress: Vec<px_io::gif::GifFrame> = Vec::new();
    ...
    if let Some(path) = &args.progress {
        let report = px_io::gif::write_progress(path, &progress)
...
/// **飛ばしたことは黙らない**．
fn capture(root: &Path, step: &ResolvedStep, delay_ms: u32) -> Option<px_io::gif::GifFrame> {
    for rel in &step.outputs {
        ...
        return Some(px_io::gif::GifFrame {
            canvas: canvas.clone(),
...
fn evaluate_fields(
    ...
    for (key, field) in &source.fields {
        out.insert(
            key.clone(),
            px_recipe::recipe::eval_field_pub(field, &env, preds)?,
        );
    }
    Ok(out)
}
...
/// 外部データからレシピを作る (設計書 4.2 «外部データ参照は px recipe expand に分離する»)．
///
...
fn expand(args: &ExpandArgs) -> Result<()> {
    ...
    let text = format!(
        "# px recipe expand が {} から書き下した ({} 行 x {} ステップ)\n{body}",
        args.data.display(),
        rows.len(),
        template.len()
    );
