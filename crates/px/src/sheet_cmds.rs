//! `px sheet pack` — スプライトシートと JSON メタ (設計書 5 章 `op = "sheet.pack"`)．
//!
//! **並べ方を決めるのはここだけである．** `px export tiled` は `--sheet` でこの
...
use px_core::sheet::{PackOptions, SheetItem, pack};
...
pub struct PackArgs {
    ...
    ///
    /// **`.json` にしない．** `px tileset extract --map` の正規 JSON が
...
fn pack_cmd(args: &PackArgs) -> Result<()> {
    ...
    if let Ok(existing) = std::fs::read_to_string(&meta)
        && px_core::sheet::SheetDoc::from_json(&existing).is_err()
    {
        bail!(
            "{} は既にあり，シートのメタではない．\n\
...
/// レイヤを下から順に載せて 1 枚にする．
fn flatten(frame: &px_core::Frame) -> Option<px_core::IndexedCanvas> {
    let first = frame.layers.first()?.surface.as_indexed()?;
    let transparent = first.transparent().unwrap_or(0);
    let mut out = px_core::IndexedCanvas::filled(frame.size.x, frame.size.y, transparent)
        .with_transparent(Some(transparent));
    for layer in &frame.layers {
        if let Some(c) = layer.surface.as_indexed() {
            out.blit(c, px_core::ivec2(0, 0), true);
        }
    }
    Some(out)
}
