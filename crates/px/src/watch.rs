//! `px watch` — 保存を検知して描き直す (M1)．
//!
//! 完了条件は「**保存から端末表示まで 1 秒未満**」なので，毎回かかった時間を
//! 表示して測れるようにしてある．
//!
//! ディレクトリを監視するのが要点である．`px` の書き出しは一時ファイルを作って
...
fn draw(path: &Path, opts: &RenderOptions, frame_index: usize, since: Option<Instant>) {
    ...
    match crate::load_frames(path) {
        Ok(frames) => match frames.get(frame_index) {
            Some(frame) => {
                let img = px_view::render::to_rgba_image(frame, opts);
                if let Err(e) = px_view::show(&img, Placement::default()) {
                    eprintln!("表示に失敗: {e}");
                    return;
                }
