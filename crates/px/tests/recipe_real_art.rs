...
//!
//! **本物の `px` を起こして測る** — `RAYON_NUM_THREADS` はプロセスの始めにしか
...
fn project(name: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!("px-recipe-{}-{name}", std::process::id()));
...
fn moving_the_whole_project_keeps_the_cache_warm() {
    ...
    run(&from, None, &[]);
    let to = std::env::temp_dir().join(format!("px-recipe-{}-move-to", std::process::id()));
...
///
/// `px shade` は L0 の隣に `.hex` も書く (D127) ．レシピが `output` に書いて
...
fn a_cache_hit_restores_a_tree_that_can_actually_be_read() {
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_px"))
        .arg("lint")
...
fn an_ambiguous_target_is_refused_with_the_choices_listed() {
    ...
    let out = root.join("progress.gif");
    let result = Command::new(env!("CARGO_BIN_EXE_px"))
        .arg("run")
