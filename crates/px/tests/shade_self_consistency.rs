...
use px_core::{Rgba8, canvas::RgbaCanvas};
...
/// **`px shade` の出力が `px lint` の blocking を 1 件も出さない．**
///
...
/// **`px shade` → `px smooth` → `px lint` が通る．**
///
...
fn smoothing_a_shaded_sprite_still_passes_the_lint() {
    ...
    for (name, img) in &seeds {
        ...
        let mut lint = px_lint::rules::lint_palette(&palette, &cfg);
        lint.extend(px_lint::lint_canvas(&smoothed, &palette, &cfg));
...
/// **`px shade` の出力を «その光源で» 検査すると，ルール 7 に落ちない．**
///
/// ルール 7 (反転同値の陰影不整合) は光源が宣言されたときだけ働く．宣言する側が
/// まさに `px shade` なので，**自分の出力が自分の検査を通ること**が最低条件である．
...
fn shading_agrees_with_its_own_light_and_a_mirror_does_not() {
    ...
    for (_name, img) in &seeds {
        ...
        for preset in LightPreset::ALL {
            ...
            if !matches!(source, px_core::ramp::LightSource::Directional { .. }) {
                continue;
            }
            ...
            checked += 1;
            if px_lint::lint_canvas(&canvas, &palette, &cfg)
                .violations
                .iter()
                .any(|v| v.rule == 7)
            {
            ...
            mirrors += 1;
            if px_lint::lint_canvas(&flipped, &palette, &cfg)
                .violations
                .iter()
                .any(|v| v.rule == 7)
            {
    ...
    assert_eq!(
        upright_fired, 0,
        "px shade の出力が自分の光源でルール 7 に落ちた ({upright_fired} / {checked})"
    );
    assert_eq!(
...
/// **`px shade` の出力がルール 14 (AA 過多) に落ちない．**
///
/// 陰影の «段» は端の 2 色の間にあり，端より狭く使われる — ルール 14 の «中間色» の
/// 定義にそのまま当てはまる．**閾値を良い絵だけで決めると自分の出力が鳴る**
/// (実測で 38.8%) ．D58 ・D77 と同じ作法で `px shade` の出力を第 3 群として扱い，
/// その上に閾値を置いてある．
///
...
fn shading_does_not_trip_the_too_much_antialiasing_rule() {
    ...
    for (name, img) in &seeds {
        ...
        for preset in LightPreset::ALL {
            ...
            let opaque = canvas
                ...
                .max(1);
            let ratio =
                px_core::aa::intermediate_pixels(&canvas, &palette, cfg.intermediate_tolerance)
                    as f32
                    / opaque as f32;
    ...
    assert_eq!(
        fired, 0,
        "px shade の出力が自分の AA 過多検査に落ちた ({fired} / {checked})"
    );
}
