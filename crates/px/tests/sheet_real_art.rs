//! **`px sheet pack` を実素材に掛けて «壊していないか» を測る．**
//!
...
use px_core::sheet::{PackOptions, SheetItem, choose_columns, pack};
...
fn packing_is_deterministic_and_the_layout_matches_what_the_rule_says() {
    ...
    let back = px_core::sheet::SheetDoc::from_json(&da.to_json().expect("書ける")).expect("読める");
    assert_eq!(back, da);
}
...
fn too_many_colours_for_one_sheet_is_refused_with_the_count() {
    ...
    let px_core::error::CoreError::SheetTooManyColors { colors, items } = err else {
        panic!("別のエラーで落ちた: {err}");
    };
