...
//! > `px-calib tileset` で測ると，種の 61 枚では 16 画素で反転に頼る升が **0 個**，
...
fn a_shaded_tile_next_to_its_mirror_is_caught_by_rule_7() {
    ...
    for (tx, ty) in &cells {
        ...
        let agreement = px_lint::rules::shading_agreement(&cell, &palette, light)
...
fn an_eight_pixel_tile_can_never_be_measured_by_rule_7() {
    ...
    assert!(
        px_lint::rules::shading_agreement(&tile, &palette, light).is_none(),
