...
use pxsmith_core::smear::{SmearMethod, SmearOptions, smear_mask};
...
fn pairs_with_two_intermediates(palette: &Palette, tolerance: f32) -> (usize, usize) {
    use pxsmith_core::color::{Oklab, distance_sq};
...
///
/// ランプの宣言が要るので，**`pxsmith shade` に描かせた絵を使う** — 実素材の
/// シルエットを取り，宣言した光源で陰影を付ければ «どの色がどのランプの
/// 何段目か» が分かっている状態を作れる (D77 が第 3 群として `pxsmith shade` の
/// 出力を測ったのと同じやり方) ．
