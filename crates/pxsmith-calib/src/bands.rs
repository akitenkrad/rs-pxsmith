...
//!
//! 不一致率の定義は `pxsmith_core::grid` の再構成検査と同じ — 画素とそのセルの平均色の
...
use pxsmith_core::{IVec2, Rgba8, RgbaCanvas};
use rayon::prelude::*;
...
/// セルの平均色．`pxsmith_core::grid` と同じく `u8` の平均を採ってから OKLab へ移す．
...
/// セル内分散の和．`pxsmith_core::grid` と同じく `u64` で厳密に積む (設計書 6.15 規則 3)．
