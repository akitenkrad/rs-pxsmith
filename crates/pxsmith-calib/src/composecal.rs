...
//!
//! 3 つ目が `pxsmith-calib aa` ・`outline` と同じ «自分の道具が自分の検査に落ちていないか»
//! の口である (D58) ．
...
use pxsmith_core::compose::{Alignment, ComposeOptions, Part, compose};
use pxsmith_core::frame::{Frame, FrameKind, Layer, LayerMeta, Surface};
use pxsmith_core::math::{IVec2, ivec2, uvec2};
...
pub fn merges(dir: &Path, bases: &[String], equips: &[String]) -> Result<Vec<Merge>> {
    let mut out = Vec::new();
    for b in bases {
        let Ok(bimg) = pxsmith_io::png::read_rgba(dir.join(format!("{b}.png"))) else {
            continue;
        };
        let bc = used_colors(&bimg);
        for e in equips {
            let Ok(eimg) = pxsmith_io::png::read_rgba(dir.join(format!("{e}.png"))) else {
                continue;
            };
...
fn used_colors(img: &pxsmith_core::canvas::RgbaCanvas) -> Vec<u32> {
...
/// ことになる．
fn blocking_of(frame: &Frame, cfg: &pxsmith_lint::LintConfig) -> (usize, BTreeMap<u8, i64>) {
    let Some(flat) = flatten(frame) else {
        return (0, BTreeMap::new());
    };
    let mut r = pxsmith_lint::rules::lint_palette(&frame.palette, cfg);
    r.extend(pxsmith_lint::lint_canvas(&flat, &frame.palette, cfg));
...
/// レイヤを下から順に載せて 1 枚にする．
fn flatten(frame: &Frame) -> Option<pxsmith_core::canvas::IndexedCanvas> {
    let first = frame.layers.first()?.surface.as_indexed()?;
    let transparent = first.transparent()?;
    let mut out =
        pxsmith_core::canvas::IndexedCanvas::filled(frame.size.x, frame.size.y, transparent)
