...
//! | **`pxsmith shade`** | 同じ形に `pxsmith shade` で陰影を付けたもの | **鳴らない (D58)** |
//!
...
fn measure(img: &RgbaCanvas, file: &str) -> Option<Record> {
    ...
    let rho = pxsmith_lint::rules::pillow_correlation(&canvas, &palette)?;
...
/// 絵の中で最も広い不透明な色 (`pxsmith shade --base` に渡す固有色にする)．
...
/// **同じ形に `pxsmith shade` で陰影を付けて測る** (D58 の群)．
...
pub fn run(
    ...
    let shaded: Vec<Record> = files
        .par_iter()
        .flat_map(|p| match pxsmith_io::png::read_rgba(p) {
