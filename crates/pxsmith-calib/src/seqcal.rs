//! **フレーム間ルール (22 〜 27) の適用範囲と閾値を測る** (`pxsmith-calib lintseq`)．
//!
...
//! | `tween` | 2 枚から `pxsmith anim tween` で中割りを作った 5 コマ | **標数は保証されない** (設計書 6.9．D114 で実測) |
...
use pxsmith_core::tween::{TweenAlign, TweenOptions, tween_series};
use pxsmith_lint::rules::LintConfig;
use pxsmith_lint::{SequenceCoverage, lint_sequence};
...
/// `pxsmith anim tween` の中割りを含む列．**中の 3 枚に `kind = inbetween` を付ける**．
...
/// **道具が作った潰し** (正例)．`pxsmith anim squash` の出力が自らの検査に落ちないか．
pub fn squash_sequence(file: &str, canvas: &IndexedCanvas, palette: &Palette) -> Option<Sequence> {
    ...
    for a in amounts {
        let (c, _) =
            pxsmith_core::deform::squash(&base, a, &pxsmith_core::deform::SquashOptions::default())
                .ok()?;
    ...
    for f in frames.iter_mut() {
        ...
        let grown = c.crop(pxsmith_core::math::IRect { x: 0, y: 0, w, h }, fill);
        f.size = uvec2(w, h);
