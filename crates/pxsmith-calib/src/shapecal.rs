//! **ルール 19 ・20 ・21 の閾値を測る** (`pxsmith-calib lint-shape`)．
//!
...
use pxsmith_core::geom::regions::label_regions;
...
fn roughen(canvas: &IndexedCanvas, palette: &Palette, seed: u64) -> Option<IndexedCanvas> {
    ...
    let inside = |p: pxsmith_core::math::IVec2| map.at(p).map(|r| r.id) == Some(target.id);
    let neighbours = [(1, 0), (-1, 0), (0, 1), (0, -1)];
    for p in target.bbox.iter() {
        let here = canvas.get(p.x, p.y);
        let touches = neighbours
            .iter()
            .any(|(dx, dy)| inside(pxsmith_core::math::ivec2(p.x + dx, p.y + dy)));
        if inside(p) {
            // **出す** — 縁の画素を隣の色にする
...
fn silhouette_row(
    ...
    for y in 0..canvas.height() as i32 {
        for x in 0..canvas.width() as i32 {
            ...
            m.set(pxsmith_core::math::ivec2(x, y), true);
        }
    }
    if m.is_empty() {
        return None;
    ...
    for p in only.iter_set() {
        for (dx, dy) in [(1, 0), (-1, 0), (0, 1), (0, -1)] {
            if !only.get(pxsmith_core::math::ivec2(p.x + dx, p.y + dy)) {
                perimeter += 1;
            }
        }
    }
...
pub fn add_tangent(canvas: &IndexedCanvas, palette: &Palette, size: i32) -> Option<IndexedCanvas> {
    ...
    let belongs =
        |x: i32, y: i32| map.at(pxsmith_core::math::ivec2(x, y)).map(|r| r.id) == Some(target.id);
