...
//! **`pxsmith validate` の «表示時間は表示周期の倍数か» (D40) に掛けて決めた．**
//!
...
//! > という事実であり，`pxsmith validate` はそれを言うべきである．
//!
//! # 周期アニメは «何を変調するか x どう変調するか» (6.12 ・D24)
//!
//! $4 \times 4 = 16$ 通り．**うち 12 通りを書き，4 通り ([`ModTarget::Rotate`])
//! は書いていない** — 回転は `pxsmith rotate` (設計書 6.13) の仕事で，まだ無い．
...
/// splitmix64 を状態なしの混ぜ合わせとして使う (`pxsmith-calib` の `rng` と同じ構成) ．
...
impl CycleSpec {
    ...
    /// 換気扇．**書いていない** — 回転は `pxsmith rotate` の仕事である．
...
mod tests {
    ...
    ///
    /// 決め手は `pxsmith validate` の «表示時間は表示周期の倍数か» である．
    ...
    ///
    /// 回転は `pxsmith rotate` (設計書 6.13) の仕事である．**書いていないと言う．**
    #[test]
