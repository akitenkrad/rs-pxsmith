...
//! | **色を作らない．元の色と空を結ぶ線の上にある色だけを使う** | D94 ・D124 — 並べ替えるだけの道具は色を作ってはいけない．«間» の定義は `pxsmith aa` (D83) と 1 つに揃える |
...
//! 概念が無い (`pxsmith-io/src/document.rs` の `project_meta`) ．しかも L0 は
//! **1 ファイル 1 レイヤ** (D9) なので，`[meta] depth` は**ファイル 1 枚に 1 つ**
//! である．これは多重スクロールの単位そのものなので，`pxsmith atmos` は
...
//! 最も近いもの．許容は `pxsmith aa` ・`pxsmith anim subpixel` と同じ値を引いて使う
...
//! 飽和する) ．だから**別の判断から引く** — `pxsmith aa` (D83) ・`pxsmith anim subpixel`
...
///
/// `pxsmith aa` の [`crate::aa::nearest_between`] と同じ条件だが，あちらは両端とも
...
pub struct AtmosOptions {
    ...
    pub sky: Rgba8,
    pub haze: HazeTable,
    /// «線の上» と認める遠回りの許容．`pxsmith aa` (D83) と同じ値を引く．
    pub tolerance: f32,
}

impl AtmosOptions {
    /// «線の上» と認める遠回りの許容の既定．**`pxsmith aa` (D83) ・`pxsmith anim subpixel`
