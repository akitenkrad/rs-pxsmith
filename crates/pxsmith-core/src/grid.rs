...
impl Default for GridParams {
    fn default() -> Self {
        Self {
            ...
            // > `pxsmith conform` が実際に受け取る入力に最も近い枠であり，D66 の要件は
            ...
            // `pxsmith-calib sweep|real --edge-fit-curve-residual` で掛け替えて比べられる
...
/// **入力が違うので道具も分かれる** (D172) — `pxsmith lint` が受け取るのは劣化していない
/// 等倍の PNG なので厳密判定が使えるが，`pxsmith conform` が受け取るのは JPEG や補間を
/// 通った絵なので使えない．
///
...
/// > `pxsmith lint` は渡された PNG が等倍か拡大かを知らないので，
/// > **絞りを規則の側に置く**しかない．一様に拡大された絵の «格子が場所により
/// > 違う» はルール 2 ・`pxsmith conform` の持ち場である．
///
...
/// 格子に沿って最頻色へ縮小する (`pxsmith conform` の中核)．
///
