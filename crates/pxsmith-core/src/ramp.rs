...
/// 光源の型 (設計書 3.3)．
///
/// `serde` を付けてあるのは，**lint の設定 ([`pxsmith_lint`] のルール 7) が光源を
/// 宣言として受け取る**ためである．
...
impl LightPreset {
    ...
    /// **このプリセットが «空の色» とみなしている色相**．
    ///
    /// 影の色相をそのまま返す — このモジュールの陰影モデルは «影は空の色»
    /// (`Clear` の説明どおり) を前提に組んであり，`shade_hue_at` の注記も
    /// «光源色と空の色» と書いている．[`crate::atmos`] が宣言された空と
    /// **突き合わせる**ために引く．
    ///
    /// > [!warning] **色相しか引けない．**
    /// > 明度と彩度はプロファイルに無いので，ここから «空の色» を作ると
    /// > 2 つの数を発明することになる．だから `pxsmith atmos` は `--sky` を要求し，
    /// > これは**並べて見せるだけ**に使う (D89 と同じ切り分け)．
    pub fn sky_hue(self) -> f32 {
        self.profile().1
    }
