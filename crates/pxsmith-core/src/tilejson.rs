...
//!
//! 実装を進める間に `pxsmith tileset autotile --map` は «bitmask → 添字» を，
//! `pxsmith tileset extract --map` は «升 → 添字 + 反転» を，**それぞれ別の形で**
//! 書いていた．**正規出力が 2 つあるのは正規出力が無いのと同じ**なので，
//! 1 つの型にまとめた．
//!
//! | 節 | 何を持つか | 誰が書くか |
//! | --- | --- | --- |
//! | `terrain` | **bitmask → タイル** (設計書 4.4 の正規形) | `pxsmith tileset autotile` |
//! | `map` | **升 → タイル** (絵をタイルへ分解した結果) | `pxsmith tileset extract` |
//!
...
impl TilesetDoc {
    ...
    /// `pxsmith tileset autotile` の出力を載せる．
    ...
    /// `pxsmith tileset extract` の出力を載せる．
