...
//! (`pxsmith anim tween` が [`crate::shade`] を呼ぶ) ．ここは形だけを引き受ける．
//!
...
//! `pxsmith-calib tween` で真値のある場面 (平行移動) を作って測ってある．
//!
...
/// 形だけを補間する (設計書 6.9)．
///
/// `t` は $[0, 1]$．外挿は `pxsmith anim extrapolate` の仕事なので受け付けない．
...
/// 呼ぶ側が測って決める．
///
/// # 平行移動なら真値があるので測れる (`pxsmith-calib extrapolate`)
///
...
// 数え方は `geom::topology` に 1 つだけ置く (D110) — `pxsmith lint` のルール 22 が
// 同じ量を見るためである
...
mod tests {
    ...
    /// 外挿は `pxsmith anim extrapolate` の仕事である．
    #[test]
