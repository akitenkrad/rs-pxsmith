...
//!
//! `ParseAndValidate` は [`pxsmith_io::l0::L0Document::parse`] + `to_frames`，
//! `Lint` は `pxsmith_lint` の 27 ルール，`ToPromptHint` はその報告文である．
...
//! (`pxsmith_io::l0::L0PaletteSpec`) ．したがって道具が宣言されたパレットから
...
fn parse_and_lint(raw: &str, path: &Path) -> Attempt {
    ...
    for frame in &frames {
        let mut report = pxsmith_lint::rules::lint_palette(&frame.palette, &cfg);
        for layer in &frame.layers {
            if let pxsmith_core::frame::Surface::Indexed(canvas) = &layer.surface {
                report.extend(pxsmith_lint::lint_canvas(canvas, &frame.palette, &cfg));
            }
        }
    ...
    //
    // ここが抜けていた (D165) — `pxsmith lint` は 22 〜 27 を掛けるのに生成の輪は
    // 掛けていなかったので，**道具が «通った» と言った列が自分の検査に落ちる**
    // 状態だった (6 ルール中 5 つが blocking である — 27 だけ advisory)．
    // `pxsmith gen prog --frames 8` で出した絵がまさにそれに当たる．
    if frames.len() >= 2 {
        let (report, _coverage) = pxsmith_lint::lint_sequence(&frames, &cfg);
        for f in report.blocking() {
            blocking.push(format!("ルール {} ({}): {}", f.rule, f.name, f.message));
        }
        advisory += report.advisory().count();
...
mod tests {
    ...
    ///
    /// `pxsmith lint` は 22 〜 27 を掛けるのに，生成の輪は 1 コマずつしか見て
