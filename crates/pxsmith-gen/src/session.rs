...
//!
//! L0 のパレットは外の `.hex` への参照なので (`pxsmith_io::l0::L0PaletteSpec`)，
...
impl Session {
    ...
    pub fn prepare(dir: &Path, stem: &str, req: &GenRequest) -> Result<Self> {
        ...
        pxsmith_io::hex::write(&hex, &palette)?;
        Ok(Self {
            dir: dir.to_path_buf(),
            stem: stem.to_string(),
        })
    ...
    fn write_provenance(
        ...
        let p = Provenance {
            tool: format!("pxsmith-gen {}", env!("CARGO_PKG_VERSION")),
