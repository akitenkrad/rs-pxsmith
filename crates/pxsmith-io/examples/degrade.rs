...
//!
//! ```sh
//! cargo run -p pxsmith-io --example degrade -- <入力> <出力.png> \
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let src = if input.extension().and_then(|e| e.to_str()) == Some("png") {
        ...
        let doc = pxsmith_io::Document::read(&input)?;
        let frame = doc.project(pxsmith_io::FrameId(0))?;
        let canvas = frame
            .layers
            .iter()
    ...
    let pixels: Vec<pxsmith_io::pxsmith_core::Rgba8> = cropped
        .pixels()
        .map(|p| pxsmith_io::pxsmith_core::Rgba8::new(p.0[0], p.0[1], p.0[2], p.0[3]))
        .collect();
    let out = pxsmith_io::pxsmith_core::RgbaCanvas::from_pixels(
        cropped.width(),
        cropped.height(),
        pixels,
    )?;
