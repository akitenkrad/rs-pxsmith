...
use pxsmith_core::frame::{Layer, LayerMeta, Surface, TileGrid, TileRef, TilesetId};
use pxsmith_core::math::{IVec2, UVec2, ivec2, uvec2};
use pxsmith_core::{Frame, FrameId, Palette, Rgba8};
...
impl Document {
    ...
    ///
    /// L0 テキストからの取り込み (`pxsmith text import`) に使う．保持層に相当する元が
    ...
    fn write_surface(&mut self, layer_index: usize, frame: usize, surface: &Surface) -> Result<()> {
        ...
        match surface {
            Surface::Indexed(canvas) => {
                ...
                let rect = canvas
                    .opaque_bbox()
                    .unwrap_or(pxsmith_core::IRect::new(0, 0, 1, 1));
...
mod tests {
    ...
    use pxsmith_core::{IndexedCanvas, Rgba8};
