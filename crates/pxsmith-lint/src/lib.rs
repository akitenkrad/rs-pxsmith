//! `pxsmith-lint` — 品質検査 (設計書 7 章)．
//!
//! lint は 2 つの役割を持つ．人の目の代わりに形式的性質を検査する役割と，
//! **生成 AI ループの評価関数**としての役割である．後者のために `pxsmith-gen` からも
...
//! | `sequence` | フレーム列全体 |
//!
//! ジャギー・AA 系を `keyframe` に限定するのが要点である．これがないと `pxsmith anim` の
//! 出力が自らの lint に大量に落ちる．
//!
//! # 実装済みのルール
//!
...
use pxsmith_core::math::{IRect, IVec2};
...
pub const RULES: &[Rule] = &[
    ...
    // 止めない** — 直す道具 (`pxsmith smooth`) はあるので，報告して人に渡す
    Rule {
        id: 8,
        name: "ジャギー",
        severity: Severity::Advisory,
