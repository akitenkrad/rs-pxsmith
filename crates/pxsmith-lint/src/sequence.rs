...
//! 多い — `pxsmith anim tween` はトポロジー変化を数え (22) ，`pxsmith anim subpixel` は
//! 孤立列を直し (25) ・除外レイヤを守り (26) ，`pxsmith anim squash` は体積の誤差を
...
//! | 25 | 新しい列のドット数 | **`pxsmith anim subpixel` の下限から引く** (D124) |
//! | 26 | 除外マスクで動いた画素 | **数え上げ．校正しない** |
//! | 27 | 体積の誤差 | **`pxsmith anim squash` の実測から引く** (D123) |
//!
...
use pxsmith_core::geom::topology::euler_characteristic;
use pxsmith_core::geom::{Mask, mask::Field};
...
/// 不透明な画素の重心 = **列の «軌跡»**．
///
/// **自前で書かない** — `pxsmith anim tween` ・`smear` ・`extrapolate` が使っている
/// [`pxsmith_core::tween::centroid`] をそのまま呼ぶ．重心の取り方が 2 つあると
/// «道具が動かした量» と «検査が見る量» が食い違う (D110)．
fn centroid(mask: &Mask) -> Option<IVec2> {
    (!mask.is_empty()).then(|| pxsmith_core::tween::centroid(mask))
}
...
/// **45 件中 11 件が両端のどちらとも違う**ことを確かめた．道具 (`pxsmith anim tween`)
...
/// > `pxsmith anim squash` が作った潰し (0 → −0.1 → −0.2 → −0.1 → 0) は**設計上
...
fn rule_23_wobble(masks: &[Mask], cfg: &LintConfig, report: &mut Report) {
    ...
    let boxes: Vec<Option<pxsmith_core::math::IRect>> = masks.iter().map(|m| m.bbox()).collect();
...
///
/// 数え方は `pxsmith anim subpixel` の `fix_isolated_columns` と**同じにする** —
/// «新しい列» は «元は透明で今は不透明になった画素の，縦に続く一かたまり» で
/// あって «列に何個あるか» ではない．直す側と検査する側で定義が違うと，
/// **道具が直した絵が検査に落ちる** (D110)．下限も
/// [`pxsmith_core::subpixel::DEFAULT_MIN_RUN`] から引く．
///
...
fn rule_25_orphan_column(masks: &[Mask], cfg: &LintConfig, report: &mut Report) {
    ...
    for t in 1..masks.len() {
        ...
        // ルール 27 の持ち場である．外さないと `pxsmith anim squash` の出力が
...
///
/// **閾値は `pxsmith anim squash` の実測から引く** — 画素は整数なので体積は保存
...
mod tests {
    ...
    use pxsmith_core::frame::{FrameKind, Layer, LayerMeta};
