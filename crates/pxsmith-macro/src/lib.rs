//! `pxsmith-macro` — L0 テキストビットマップを Rust に埋め込む `pixels!` (設計書 4.1)．
//!
//! # なぜ proc-macro か
//!
...
//! # 2 つの書き方
//!
//! どちらも `Vec<pxsmith_core::Frame>` へ展開される．L0 は 1 ファイル = 1 レイヤ分の
...
//! # 前提
//!
//! 展開結果は `pxsmith_core` の型を名指しするので，**呼び出し側が `pxsmith-core` に依存している
...
/// L0 テキストビットマップを `Vec<pxsmith_core::Frame>` へ展開する．
///
/// 詳しい書き方はクレートの説明を参照．
#[proc_macro]
