//! `pxsmith direction` — 方向展開 (設計書 4.3 ・L2 «8 方向»)．
//!
//! 設計書 4.3 は «**自動ミラーで生成したタイルには lint ルール 7 を blocking で
//! 適用する**» と定める．**その適用側がここである** — `pxsmith-core` は絵を作るだけで，
//! 判定は lint を呼べるこちらが持つ．
//!
...
use clap::Args;
use pxsmith_core::compose::expand_template;
...
use pxsmith_core::ramp::{LightPreset, LightSource};
...
pub struct DirectionArgs {
    ...
    /// > (`pxsmith-calib direction`) ．`pxsmith shade` で作った素材のための道具である
    #[arg(long)]
    pub reshade: bool,
...
pub fn direction_cmd(args: &DirectionArgs) -> Result<()> {
    ...
    let mode = if args.reshade {
        let base = match &args.base {
            Some(hex) => pxsmith_core::color::Rgba8::from_hex_str(hex)
        ...
        ExpandMode::Reshade(Box::new(ReshadeSpec {
            ...
            shade: pxsmith_core::shade::ShadeOptions::default(),
        }))
    } else {
        ExpandMode::Mirror
    };
    ...
    for g in &report.generated {
        ...
        if checkable {
            for frame in frames {
                hits += pxsmith_lint::lint_frame(frame, &cfg)
                    .blocking()
                    .filter(|v| v.rule == 7)
                    .count();
            }
    ...
    match (light, checkable) {
        ...
        (Some(l), false) => {
            let lx = pxsmith_core::outline::light_direction(l).x;
            println!(
