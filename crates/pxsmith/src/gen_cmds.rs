...
//!
//! **書いたのは経路 B (`pxsmith gen prog`) だけである** (D156) ．`pxsmith gen image` は
...
use pxsmith_gen::anthropic::AnthropicGenerator;
use pxsmith_gen::request::{Backend, Constraints, Effort, GenKind, GenRequest};
use pxsmith_gen::session::{Session, describe_frames, now_iso8601};
use pxsmith_gen::{Attempt, Generator};
...
pub struct ProgArgs {
    ...
    #[arg(long, default_value_t = pxsmith_gen::anthropic::DEFAULT_MAX_TOKENS)]
    pub max_tokens: u32,
    /// 叩かずに，何をどう頼むかだけ出す
    #[arg(long)]
    pub dry_run: bool,
...
fn prog(args: &ProgArgs) -> Result<()> {
    ...
    if args.dry_run {
        // **組み立てた依頼をそのまま出す** (D162) — 手引きは前からこれを
        // 約束していたのに，出ていたのは上の 4 行だけだった．鍵はヘッダに
        // しか無いので本文を出しても漏れない
        println!("\n  組み立てた依頼 (鍵はヘッダにあり，本文には無い):");
        println!(
            "{}",
            pxsmith_gen::anthropic::preview_request(&req, &session.palette_ref())
        );
        println!(
        ...
        Some(v) => {
            ...
            println!(
                "    ** 通ったのは blocking が 0 だからで，advisory {} 件は残っている ** — \n\
                 ...
                 \u{3000}\u{3000}`pxsmith lint` で中身を見ること",
                v.advisory
            );
