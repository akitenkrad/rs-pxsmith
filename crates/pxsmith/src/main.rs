...
use pxsmith_core::Frame;
use pxsmith_core::frame::FrameKind;
use pxsmith_io::hex;
use pxsmith_io::l0::{L0Document, Violation};
...
pub(crate) enum Command {
    ...
    Conform {
        ...
        ///
        /// **倍率の 4 倍に取ること** (D164)．窓の一辺にセルが 4 つ入らないと局所推定は
        /// 当たらず，大きく取りすぎると窓が減って 1 つの過大推定が一致率を割る．
        /// 既定の 32 が見られるのは s <= 8 までである
    ...
    /// 生成 AI 連携 (設計書 8 章)．**`pxsmith gen image` は書いていない** (D156)
    Gen {
        #[command(subcommand)]
...
impl DisplayArgs {
    fn options(&self) -> RenderOptions {
        RenderOptions {
            ...
            silhouette: self
                .silhouette
                .then_some(pxsmith_core::Rgba8::rgb(0x1a, 0x1c, 0x2c)),
            ..RenderOptions::default()
        }
    }

    fn onion(&self) -> pxsmith_view::onion::OnionOptions {
        let before = self.onion.unwrap_or(0);
        pxsmith_view::onion::OnionOptions {
            before,
...
/// サブコマンドを実行する．
///
/// **`main` から切り出してあるのは `pxsmith run` が同じ道を通るためである** —
...
/// > `.aseprite` を書いていた — `pxsmith shade out.px.toml` が**中身は `.aseprite` の
/// > ファイル**を作り，それを `pxsmith anim smear` に渡すと «UTF-8 でない» で落ちる．
...
/// > `.aseprite` にはその欄が無いので，`.aseprite` で書き出すと `pxsmith lint` は
/// > 中割りにもジャギーを鳴らす．
/// >
    ...
    if frames.iter().any(|f| f.kind != FrameKind::Key) {
        println!(
            "  ** .aseprite は kind を持てない ** — 書いた中割りは読み直すと key に\n\
             なるので，pxsmith lint のスコープ (D47) が効かない．\n\
    ...
    match command {
        TextCommand::Export {
            ...
            if !palette_path.exists() {
                pxsmith_io::hex::write(&palette_path, &first.palette)
...
fn view(path: &Path, display: &DisplayArgs) -> Result<()> {
    ...
    let (img, report) =
        pxsmith_view::onion::onion_image(&frames, display.frame, &onion, &display.options());
    pxsmith_view::show(&img, pxsmith_view::term::Placement::default())?;
    println!(
        "{} フレーム {} / {} — {}x{}，{} ms，{}",
        path.display(),
        display.frame,
...
fn diff(a: &Path, b: &Path, limit: usize) -> Result<()> {
    ...
    for (i, (x, y)) in fa.iter().zip(&fb).enumerate() {
        ...
        for (j, (lx, ly)) in x.layers.iter().zip(&y.layers).enumerate() {
            ...
            let d = pxsmith_view::diff::diff_indexed(cx, cy)?;
            if d.is_empty() {
                continue;
            }
            total += d.count();
            println!(
                "フレーム {i} / レイヤ {j} '{}'\n{}",
                lx.meta.name,
                pxsmith_view::diff::format(&d, limit)
            );
        }
    }
    if total == 0 {
