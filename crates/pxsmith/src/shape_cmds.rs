...
use pxsmith_core::frame::Surface;
use pxsmith_core::geom::Mask;
use pxsmith_core::guide::GuideOptions;
use pxsmith_core::outline::{OutlineOptions, OutlineStyle, outline};
use pxsmith_core::project::{Facing, ProjectOptions, Projection, SourcePlane, Step};
use pxsmith_core::ramp::{LightSource, build_lighting};
use pxsmith_core::resample::{ResampleAlgo, ResampleOptions};
use pxsmith_core::shade::{DEFAULT_AMBIENT_OCCLUSION, ShadeOptions, shade_to_canvas};
...
/// `pxsmith shade` — シルエットへ陰影を導出する (設計書 6.2)．
pub fn shade(args: &ShadeArgs) -> Result<()> {
    ...
    let source = match &args.light {
        ...
        None => pxsmith_core::ramp::LightPreset::default_source(preset),
    };
...
/// `pxsmith smooth` — ジャギーを正規化する (設計書 6.4)．
...
/// `pxsmith aa` — アンチエイリアスを付ける (設計書 6.5)．
...
pub struct OutlineArgs {
    ...
    /// 光源 (`--style shaded` と `--selective` が使う)．書式は `pxsmith shade` と同じ
    #[arg(long)]
    pub light: Option<String>,
...
/// `pxsmith outline` — 縁取りを描く (設計書 D36)．
...
pub(crate) fn parse_light(spec: &str) -> Result<LightSource> {
    ...
    let source = match (kind, n.len()) {
        ("dir", 2) => LightSource::Directional {
            dir: pxsmith_core::vec2(n[0], n[1]),
        },
        ("point", 2..=3) => LightSource::Point {
            pos: pxsmith_core::vec2(n[0], n[1]),
            intensity: intensity(2),
        },
        ("line", 4..=5) => LightSource::Line {
            a: pxsmith_core::vec2(n[0], n[1]),
            b: pxsmith_core::vec2(n[2], n[3]),
...
mod tests {
    use super::*;
    use pxsmith_core::math::Rect;
    use pxsmith_core::vec2;
...
// --------------------------------------------------------- pxsmith scale / pxsmith rotate

/// `pxsmith scale` — 拡縮する (設計書 5 章 ・D18)．
#[derive(Args, Clone)]
...
/// `pxsmith rotate` — 回転する (設計書 5 章 ・D18)．
#[derive(Args, Clone)]
...
fn report_resample(r: &pxsmith_core::resample::ResampleReport, input: &Path, output: &Path) {
    println!(
        "  {} -> {} ({}x{} -> {}x{}．流儀 {})",
        input.display(),
        output.display(),
    ...
    for frame in &frames {
        ...
        for layer in &mut next.layers {
            if let Surface::Indexed(c) = &layer.surface {
                let (n, r) = pxsmith_core::resample::scale(c, &frame.palette, args.factor, &opts)?;
                next.size = pxsmith_core::math::uvec2(n.width(), n.height());
...
pub fn rotate_cmd(args: &RotateArgs) -> Result<()> {
    ...
    for frame in &frames {
        ...
        for layer in &mut next.layers {
            if let Surface::Indexed(c) = &layer.surface {
                let (n, r) =
                    pxsmith_core::resample::rotate(c, &frame.palette, args.degrees, &opts)?;
                next.size = pxsmith_core::math::uvec2(n.width(), n.height());
                layer.surface = Surface::Indexed(n);
                last = Some(r);
            }
...
pub fn project_cmd(args: &ProjectArgs) -> Result<()> {
    ...
    for frame in &frames {
        ...
        for layer in &mut next.layers {
            if let Surface::Indexed(c) = &layer.surface {
                let (n, r) = pxsmith_core::project::project(c, &frame.palette, &opts)?;
                next.size = pxsmith_core::math::uvec2(n.width(), n.height());
                layer.surface = Surface::Indexed(n);
                last = Some(r);
            }
...
fn report_project(r: &pxsmith_core::project::ProjectReport, input: &Path, output: &Path) {
    println!(
        "  {} -> {} ({} ・{} から ・{} 向き ・段 {})",
        input.display(),
        output.display(),
...
/// `pxsmith guide` — 投影ガイドグリッドを引く (設計書 6.13)．
#[derive(Args, Clone)]
...
fn parse_size(spec: &str) -> Result<pxsmith_core::math::UVec2> {
    let (w, h) = spec
        .split_once(['x', 'X'])
        .with_context(|| format!("画布 '{spec}' を読めない (`WxH` のはず)"))?;
    Ok(pxsmith_core::math::uvec2(
        w.trim()
            .parse()
...
pub fn guide_cmd(args: &GuideArgs) -> Result<()> {
    ...
    let (canvas, palette, r) = pxsmith_core::guide::guide(&opts)?;

    let name = resample_stem(&args.output);
    let mut frame = pxsmith_core::frame::Frame::new(canvas.size(), palette);
    frame.layers.push(pxsmith_core::frame::Layer::new(
        pxsmith_core::frame::LayerMeta::named("guide"),
