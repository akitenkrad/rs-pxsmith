//! `pxsmith tileset extract` — タイル分割と重複除去 (設計書 6.7)．
//!
//! 設計書 6.7 は «縮約前後のタイル数と削減率を必ず報告する» ・«反転を有効にした
//! 場合，陰影を持つ素材では lint ルール 7 (blocking) で検出する» の 2 つを定める．
//! **どちらもここで満たす** — `pxsmith-core` は切って束ねるだけで，判定は `pxsmith-lint` を
//! 呼べるこちらが持つ (`pxsmith direction` と同じ形) ．
//!
...
use pxsmith_core::autotile::{
    ImportLayout, Piece, Quadrant, build, import_quadrants, resolve, seam_doubled,
};
use pxsmith_core::canvas::IndexedCanvas;
use pxsmith_core::frame::{Frame, Layer, LayerMeta, Surface};
use pxsmith_core::math::uvec2;
use pxsmith_core::palette::Palette;
use pxsmith_core::ramp::LightSource;
use pxsmith_core::tilejson::TilesetDoc;
use pxsmith_core::tileset::{DedupeMode, ExtractOptions, extract, mirror_reliant_cells};
...
pub struct ImportArgs {
    ...
    /// (`pxsmith tileset extract` や `pxsmith tileset autotile` の出力)
    pub input: PathBuf,
    /// 書き出す L0 (`.px.toml`)
    pub output: PathBuf,
...
fn import_cmd(args: &ImportArgs) -> Result<()> {
    ...
    let palette = frames
        ...
        .palette
        .clone();
    let pieces: Vec<pxsmith_core::canvas::IndexedCanvas> = frames
        .iter()
        .map(|f| {
            f.layers
                .iter()
    ...
    if !args.palette.exists() {
        pxsmith_io::hex::write(&args.palette, &palette)
...
fn build_l0(
    art: &pxsmith_core::autotile::QuadrantArt,
    palette: &Palette,
    args: &ImportArgs,
    layout: ImportLayout,
) -> Result<L0Document> {
    ...
    used.sort_unstable();
    used.dedup();
    if used.len() > pxsmith_io::l0::COLOR_KEYS.len() {
        bail!(
            "{} 色を使っているが L0 の上限は {} 色である",
            used.len(),
            pxsmith_io::l0::COLOR_KEYS.len()
        );
    }
    ...
    let mut map: BTreeMap<String, pxsmith_io::l0::RawColorKey> = BTreeMap::new();
    let mut keys = pxsmith_io::l0::COLOR_KEYS.chars();
    for i in &used {
        if transparent == Some(*i) {
            key_of.insert(*i, '.');
            map.insert(
                ".".to_string(),
                pxsmith_io::l0::RawColorKey::Name("transparent".into()),
            );
            continue;
        }
        let c = keys.next().context("色キーが足りない")?;
        key_of.insert(*i, c);
        map.insert(c.to_string(), pxsmith_io::l0::RawColorKey::Index(*i));
    }

    let mirrored = layout == ImportLayout::Quadrants5;
    let mut frames: Vec<pxsmith_io::l0::L0Frame> = Vec::new();
    for state in pxsmith_core::autotile::STATES {
        if mirrored {
            // 全省略 — 自動ミラーに任せる (段 2)
            let c = &art[&(Quadrant::NW, state)];
            frames.push(l0_frame(state.as_str(), None, c, &key_of));
        } else {
            for q in pxsmith_core::autotile::QUADRANTS {
                let c = &art[&(q, state)];
    ...
    Ok(L0Document {
        meta: pxsmith_io::l0::L0Meta {
            format: 1,
            name: args.name.clone(),
            kind: Some("autotile_quadrants".to_string()),
            tile_size: Some(args.tile),
            ..pxsmith_io::l0::L0Meta::default()
        },
        palette: pxsmith_io::l0::L0PaletteSpec {
            reference: relative,
            map,
        },
        frames,
...
fn l0_frame(
    ...
    pxsmith_io::l0::L0Frame {
        ...
        quadrant: quadrant.map(|q| match q {
            Quadrant::NW => pxsmith_io::l0::Quadrant::NW,
            Quadrant::NE => pxsmith_io::l0::Quadrant::NE,
            Quadrant::SW => pxsmith_io::l0::Quadrant::SW,
            Quadrant::SE => pxsmith_io::l0::Quadrant::SE,
        }),
        data,
    }
}
...
fn autotile_cmd(args: &AutotileArgs) -> Result<()> {
        ...
        .map(|(raw, frame)| {
            ...
            Ok(Piece {
                ...
                quadrant: raw.quadrant.map(|q| match q {
                    pxsmith_io::l0::Quadrant::NW => Quadrant::NW,
                    pxsmith_io::l0::Quadrant::NE => Quadrant::NE,
                    pxsmith_io::l0::Quadrant::SW => Quadrant::SW,
                    pxsmith_io::l0::Quadrant::SE => Quadrant::SE,
                }),
                art,
            })
        })
    ...
    if used_mirror && let Some(declared) = light {
        for (mask, canvas) in &tiles {
            ...
            let Some(a) = pxsmith_lint::rules::shading_agreement(canvas, &palette, declared) else {
                if pxsmith_lint::rules::shading_sample_count(canvas, &palette)
    ...
    let out_frames: Vec<Frame> = tiles
        ...
        .map(|(_, t)| Frame {
            ...
            duration_ms: 100,
            kind: pxsmith_core::frame::FrameKind::Key,
        })
        .collect();
    ...
    if let Some(path) = &args.map {
        ...
        let doc = TilesetDoc::new(tile, tiles.len()).with_terrain(tiles.iter().enumerate().map(
            |(i, (mask, _))| {
                (
                    *mask,
                    pxsmith_core::frame::TileRef {
                        id: i as u32,
                        ..pxsmith_core::frame::TileRef::default()
                    },
                )
            },
        ));
...
fn extract_cmd(args: &ExtractArgs) -> Result<()> {
    ...
    if let Some(declared) = light {
        for (tx, ty) in &cells {
            ...
            // **«鳴らない» と «測れない» を分け，«測れない» の理由も分ける** (D77)
            let Some(a) = pxsmith_lint::rules::shading_agreement(&t, &palette, declared) else {
                if pxsmith_lint::rules::shading_sample_count(&t, &palette)
                    < cfg.shading_min_pixels as usize
                {
                    too_small += 1;
                }
...
mod tests {
    use super::*;
    use pxsmith_core::autotile::{CornerState, QUADRANTS, STATES};
    use pxsmith_core::canvas::IndexedCanvas;
    ...
    fn an_opaque_index_zero_is_not_written_as_transparent() {
        ...
        let mut art = pxsmith_core::autotile::QuadrantArt::new();
        for q in QUADRANTS {
            for state in STATES {
        ...
        for (key, value) in &doc.palette.map {
            if let pxsmith_io::l0::RawColorKey::Name(name) = value {
    ...
    fn a_genuinely_transparent_index_is_written_as_transparent() {
        ...
        assert!(matches!(
            doc.palette.map.get("."),
            Some(pxsmith_io::l0::RawColorKey::Name(n)) if n == "transparent"
        ));
    }
