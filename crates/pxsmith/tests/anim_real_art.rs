//! **`pxsmith anim` を実素材に掛けて «壊していないか» を測る．**
//!
...
//! | `ease` | 設計書 6.11 の表と一致し，`pxsmith validate` の表示周期の検査を通る |
...
use pxsmith_core::anim::{CycleSpec, ModTarget, Wave, cycle, duration_ms, ease};
...
use pxsmith_core::palette::{ChromaCurve, Palette, Ramp};
use pxsmith_core::tween::{TweenAlign, TweenOptions, tween_mask, tween_series};
...
///
/// 実測 (`pxsmith-calib tween`．種 64 枚 x ずらし 4 通り = 256 件) ．
///
...
///
/// $A \cap B \subseteq R \subseteq A \cup B$ は代数から出る (`pxsmith_core::tween`) ．
...
///
/// 自分で付けた表示時間を自分の検査 (`pxsmith validate`) に掛ける (D58 と同じ場面) ．
...
/// でも `ease` の誤りでもなく，そういう事実である．**`pxsmith validate` はそれを言う
...
fn every_written_combination_is_deterministic_on_real_art() {
    ...
    for path in files.iter().take(8) {
        let img = pxsmith_io::png::read_rgba(path).expect("読める");
