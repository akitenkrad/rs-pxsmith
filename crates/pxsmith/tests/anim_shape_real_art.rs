...
use pxsmith_core::afterimage::{AfterimageOptions, afterimage};
use pxsmith_core::canvas::{IndexedCanvas, RgbaCanvas};
use pxsmith_core::color::Rgba8;
use pxsmith_core::deform::{SquashOptions, VolumeRule, squash};
use pxsmith_core::frame::{Frame, Layer, LayerMeta, Surface};
use pxsmith_core::geom::{Mask, label_mask};
...
use pxsmith_core::subpixel::{
    SubpixelMethod, SubpixelOptions, SubpixelReport, SubpixelScope, subpixel,
};
use pxsmith_core::tween::{ExtrapolateKind, TweenAlign, TweenOptions, extrapolate_mask};
...
fn indexed(path: &Path) -> Option<(IndexedCanvas, Palette)> {
    index_exactly(&pxsmith_io::png::read_rgba(path).ok()?)
}
...
fn the_trail_stays_behind_the_subject_and_invents_nothing() {
    ...
    for path in png_files() {
        for dx in [1i32, 8] {
            ...
            // 拾ってしまい，1 画素の動きでも見えてしまう (pxsmith-calib で測った表)
