...
fn auto_mirrored_tiles_contradict_a_declared_diagonal_light() {
    ...
    for (_, t) in &tiles {
        let Some(a) = pxsmith_lint::rules::shading_agreement(t, &palette, diagonal()) else {
            continue;
        };
        measured += 1;
        if a < threshold {
...
fn unmeasurable_tiles_are_symmetric_not_too_small() {
    ...
    let (tiles, _) = build(&art, 16).expect("組める");
    let palette = palette();
    let min = pxsmith_lint::LintConfig::default().shading_min_pixels as usize;

    let mut unmeasurable = 0usize;
    for (mask, t) in &tiles {
        let samples = pxsmith_lint::rules::shading_sample_count(t, &palette);
        assert_eq!(
            samples, 196,
            "16x16 の全面不透明なら標本は 196 のはずである"
        );
        assert!(samples >= min);
        if pxsmith_lint::rules::shading_agreement(t, &palette, diagonal()).is_none() {
            unmeasurable += 1;
...
fn a_quadrant_is_too_small_for_rule_7_even_when_the_tile_is_not() {
    ...
    for q in QUADRANTS {
        for state in STATES {
            let c = &art[&(q, state)];
            let samples = pxsmith_lint::rules::shading_sample_count(c, &palette);
            assert!(
                samples < min,
                "8x8 の象限で標本が {samples} — 下限 {min} に届いてしまっている"
            );
            assert!(
                pxsmith_lint::rules::shading_agreement(c, &palette, diagonal()).is_none(),
    ...
    let measurable = tiles
        .iter()
        .filter(|(_, t)| pxsmith_lint::rules::shading_agreement(t, &palette, diagonal()).is_some())
        .count();
