//! **`pxsmith compose` を実素材に掛けて «壊していないか» を測る．**
//!
//! `pxsmith-calib compose` で測ると，実素材を重ねると **lint の blocking が増える組が
...
/// PNG を色を落とさずに添字の面へ写す (`pxsmith-calib` の `index_exactly` と同じ)．
...
fn part_of(name: &str) -> Option<Part> {
    let img = pxsmith_io::png::read_rgba(seeds().join(format!("{name}.png"))).ok()?;
...
/// **壊れると: 合成が色を作る ・変える．**
///
/// 色が変わらないことが `pxsmith compose` の不変条件である．ここが崩れると，
...
///
/// 実素材のシルエットは 38 枚中 33 枚が画布の縁に接している (`pxsmith-calib compose`) ．
