...
//! サブコマンドの木から引く (D130) ので，**`pxsmith gen` に枝を足すたびに新しい op
//! が生える** — 門を完全一致で書いていると，足した枝が素通りして
//! `--allow-generate` 無しで外部 API を叩けてしまう．
//!
//! M6 で `pxsmith gen prog` を足した瞬間に実際そうなった．**枝が増えても門が
//! 閉じていること**をここで固定する．
//!
//! 本物の `pxsmith` を起こして測る — 門は CLI の側にあり，ライブラリ試験では
...
fn project(name: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!("pxsmith-gengate-{}-{name}", std::process::id()));
