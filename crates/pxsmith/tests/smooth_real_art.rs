//! **`pxsmith smooth` を本物のドット絵に掛けて «壊していないか» を見る．**
//!
//! `pxsmith-core` 側の試験は階段と円で «直ること» を見ている．ここで見るのは逆で，
...
//!
//! 全件の集計は `pxsmith-calib jaggy --apply` にある (61 枚で 104 画素 ・blocking の増加 0) ．
...
/// PNG をそのまま添字にする (`pxsmith-calib` と同じ作法．色を減らさない)．
...
fn smoothing_real_pixel_art_breaks_nothing() {
    ...
    for (name, canvas, palette) in &seeds {
        ...
        // **直した先で増やさない**
        let before_jaggies = pxsmith_core::geom::jaggy::analyze_canvas(canvas, opts.max_move)
            .jaggies
            .len();
