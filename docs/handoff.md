...
| ブランチ | `main` |
| コミット | **`9efca23` まで**．D162 〜 D165 はすべてコミット済み (**作業ツリーはきれい**) |
...
| **R3 の残り** | `aseprite-tests/` は `aseprite-io` 自身の fixtures でもあるので，**`aseprite-io` の忠実性を独立に検証したことにはならない**．最新版 Aseprite で自作したファイルを `testdata/aseprite/independent/` へ置けば潰せる |
