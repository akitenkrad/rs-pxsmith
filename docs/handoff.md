...
- 更新: 2026-08-14 (M5 完了．**M4 は 20 件中 18 件**)
...
| 項目 | 値 |
| --- | --- |
| フェーズ | M0 ・M1 ・M1a ・**M2 完了** (B は未達のまま閉じた) ・**M3 完了**．**M4 は 20 件中 18 件** (残り: `px lint` の sequence 6 ルール ・`px atmos`) — **`px compose`** (D93 〜 D95) ・**`px direction`** (D96 〜 D98) ・**`px tileset extract`** (D99 〜 D101) ・**`px tileset autotile`** (D102 〜 D104) ・**ディザの位相** (D105 〜 D107) ・**象限インポータ** (D108 ・D109) ・**正規 JSON の統合** (D110) ・**`px export tiled`** (D111) ・**`px sheet pack`** (D112 ・D113) ・**`px anim tween`** (D114 ・D115) ・**`px anim ease`** (D116 ・D117) ・**`px anim cycle`** (D118 ・D119) ・**`px anim smear`** (D120 ・D121) ・**`px anim extrapolate`** (D122) ・**`px anim squash`** (D123 ・D128) ・**`px anim subpixel`** (D124 ・D125) ・**`px anim afterimage`** (D126) ・**書く側も拡張子を見る** (D127) ．**M5 完了** — レシピ ・式評価器 ・依存グラフ ・ステップキー ・キャッシュ ・`recipe expand` ・決定論性試験 ・生成過程 GIF (D129 〜 D134) ．**次は M4 の残り 2 件** (`px atmos` ・sequence lint) |
| テスト | **772 件**すべて通過 |
...
| ブランチ | `main` |
| コミット | M4 はすべてコミット済み (`bb7b6dd` 〜 `4bb8791`) ．**M5 は未コミット** |
