...
| # | やること | なぜ | 上限 |
| --- | --- | --- | --- |
| **1** | **`px gen prog` を鍵のある環境で 1 度通す** | バックエンドを**本物の API に 1 度も当てていない** | 読める (1 回叩けば分かる) |
| 2 | 付録 C #5 を**閉じるか測り直すか決める** | D125 が実質測っているが «使える確率» として明記していない | 読める |
| 3 | 付録 C #1 (ラン長の谷の誤検出) | M3 から開いたまま | 読める |
| 4 | 付録 C #4 (局所格子推定の窓サイズ) | M2 から開いたまま | 読める |
...
**見るもの**．出た差分は `docs/status.md` の M6 節へ書く．

| 見るもの | 壊れていたら |
| --- | --- |
| 送った本文が受け取られるか | `output_config` ・`fallbacks` ・`cache_control` のどれかが弾かれている |
| 応答から L0 が取り出せるか | 構造化出力の受け方 (`content` の `text` ブロック → `l0` 欄) が違う |
| 何回で通るか | プロンプトが弱い．**輪の回数ではなくプロンプトを直す** |
| `stop_reason` の扱い | 断りが «壊れた応答» として報告されていないか |

**輪の側は偽バックエンドで試験してある**ので，落ちるとすれば
`crates/px-gen/src/anthropic.rs` の中だけである．

> [!warning] **生成の輪はプロンプトが間違っていても «モデルが下手» に見える．**
> 通らないときは，まず `--dry-run` で組み立てた依頼を読み，**次に
> `crates/px-gen/src/anthropic.rs` の `system_prompt` を `px_io::l0` の
> スキーマと突き合わせる**こと (M6 でここを記憶で書いて 1 度外している) ．
...
| 5 | `--method fast` が中間フレームとして使える確率 | **D125 が実質測っている** — 実素材 61 枚中 32 枚でシルエットが動く (= 47.5% は動かない ・blocking 増は 5 枚) ．ただし «使える確率» として明記していないので，**それを根拠に閉じるか測り直すかを決める** |
