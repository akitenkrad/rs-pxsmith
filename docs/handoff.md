...
- 更新: 2026-08-15 (**M0 〜 M7 完了．M6 は本物の API で通った (D162)．
  付録 C は #1 (D163) と #4 (D164) を閉じて，**全 5 件が閉じた**)
...
| ブランチ | `main` |
| コミット | **D162 〜 D169 はすべてコミット済み．作業ツリーはきれい** (最後のコミットは `git log -1` で見る — 番号をここに書くと自分を追いかける) |
...
| 2 | **R22 の残り — 円板** (下) | 直線は守った (D169) ．**円板 60 画素 ・13 枚が残る** | 読める |
...
| 負例の捕捉 | 53 / 69 (76.8%) | **変わらず** |
| 実素材で動かした画素 | 104 | **89** |

**残るのは円板 (60 画素 ・13 枚)．** ただし中身は «散発» ではなく**構造**だった —
鳴った円板はどれも**ちょうど 4 件**で，**90 度回転で移り合う 4 点**に出る．
形はすべて **`[2, 1, 2]`**，角度は約 28.6 度 (45 度ではないので八分円の境目ではない) ．
**«走りが 2 の帯» から «走りが 1 の帯» への継ぎ目**，つまり**傾きの違う 2 つの
直線区間の継ぎ目**である．

道具は **maximal digital straight segment 分解 (tangential cover) + 凸性**で，
`is_digital_straight` はその 1 段目の非増分版なので地続きである．

> [!warning] **直線の例外は «健全» だったが，凸弧の例外はそうならない．**
> 厳密に直線なら**定義上直すべきものが無い**ので判断の余地が無かった．
> 凸弧の判定は**弧が本当に凸であること**を要求するが，**実素材は凸でない絵が
> 大半**で，しかも **D168 のとおり局所の形では偽と真が分かれない** —
> **«継ぎ目だから許す» は本物のジャギーまで許す見込みがある．**

**次は実装より先に上限を測る (決定済み)** — «継ぎ目の谷を全部許したら» 捕捉
53 / 69 と実素材 61 枚がどうなるかを測り，**採れないなら実装せずに済ませる**．

> [!warning] **谷の局所の形では偽と真が分かれない** (D168 で測った) ．
> 深さ (偽 161 / 真 170 が深さ 1) ・両隣の差 (偽 145 / 真 137 が差 0) ・
> 区間内の位置，**3 つとも同じ分布**である．次に触るなら，局所の閾値ではなく
> **区間全体が滑らかな曲線の digitization か**を見る形になる．
