...
### 0. **M5 完了．M4 に 2 件残っている (2026-08-14)**
...
| 式評価器 (`&&` `||` `!` は書いていない) | **済 (D129)** |
| 組込み述語 6 種 | **済** |
| `px run` (依存グラフ) | **済 (D130)** — op → コマンド行は clap から引く |
| 差分ビルド (BLAKE3 ・`--no-cache` ・`--gc`) | **済 (D131 ・D132)** — 2.66 s → 0.09 s |
| 決定論性テスト (`RAYON_NUM_THREADS`) | **済** — 1 / 2 / 4 / 8 でバイト一致 |
| `px recipe expand` | **済 (D133)** — 行ごとに書き下す |
| ドキュメント (README + 使用例) | **済** |
| 生成過程 GIF | **済 (D134)** — 祖先の連鎖だけを並べる |
| `op = "gen"` | **書かない** (M6) |

> [!warning] **«M4 完了» と報告したのは誤りだった (2026-08-14)．**
> この文書が持っていた M4 の表に，**実装計画書の完了条件 20 件のうち 2 件が
> 載っていなかった** — `px lint` の sequence 6 ルールと `px atmos` である．
> **正は実装計画書の M4 節**であり，そこと突き合わせずに閉じたのが誤りだった
> (進捗表が 2 つあった — D110 と同じ形) ．

| **M4 に残っている完了条件** | 状態 |
| --- | --- |
| **`px atmos`** | **次はここ** — `depth` に基づく空気遠近法 ・多重スクロールメタ出力 |
| **`px lint` (sequence) フレーム間 6 ルール** | 未 — ルール 22 〜 27 |

**その後は M6 (生成 AI 連携，6〜10 人日) か M7 (投影 ・拡縮回転 ・確認機能，
9〜14 人日) である．M7 は他機能に依存しないので後回しでも損失がない．**
