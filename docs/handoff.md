...
- 更新: 2026-08-14 (**M0 〜 M5 ・M7 完了．M6 は 3 件済 ・1 件は書かない．
  バックエンドを本物の API に当てて 2 件直した — ただし鍵が期限切れで 200 は未確認．
...
>
> ```sh
> export ANTHROPIC_API_KEY=...   # 有効なもの．~/.zshrc:162 のは期限切れ
...
> 鍵は**期限切れ**で `401` だったので**200 の応答はまだ見ていない**．
...
| # | やること | なぜ | 上限 |
| --- | --- | --- | --- |
| **1** | **`px gen prog` を «有効な» 鍵で 1 度通す** | 当てたが**鍵が期限切れで 200 を 1 度も見ていない** | 読める (1 回叩けば分かる) |
...
```sh
export ANTHROPIC_API_KEY=...   # 有効なもの．~/.zshrc:162 のは期限切れ (401)
cargo run -p px -- gen prog out/chest.px.toml --prompt "木の宝箱．正面から" \
    --palette 1a1c2c,566c86,8a6a4a,b13e53,f4f4f4 --size 16x16
```
