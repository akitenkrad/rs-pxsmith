...
> > `px lint` は 22 〜 27 を掛け，**6 ルール中 4 つが blocking** なのに，
...
>
> **32x32 は 672 秒かかった** (16x16 の 80.4 秒の 8.4 倍) ．D162 の «無音の上限は
> 60 秒» とあわせると，**一括で受けていたら 32x32 は手が届かない**．
>
> ```sh
...
| # | やること | なぜ | 上限 |
| --- | --- | --- | --- |
| **1** | **M6 の «輪» の残り — `Refused` と «2 回目»** | `Truncated` は D165 で取った．**輪は本物の応答ではまだ回っていない** | 読める |
...
#### 1. M6 の «輪» — **通ったが，まだ回っていない** (D165 で 2 件前進した)

| 見るもの | 状態 |
| --- | --- |
| 送った本文が受理されるか | **済**．`output_config` ・`fallbacks` ・`cache_control` とも弾かれない |
| 応答から L0 が取り出せるか | **済** (流で受ける形に書き直した．D162) |
| 何回で通るか | **2 回とも 1 回目で通った** (16x16 は 80.4 秒 ・**32x32 は 672 秒**) |
| **`Truncated`** | **済 (D165)**．`--max-tokens 300` で本物の API から取った (15 秒) |
| `Refused` | **未実測**．偽バックエンドの試験のみ |
| 2 回目以降 ・助言の効き方 | **未実測**．ただし**輪が回る道は塞がっていない**ことを試験で確かめた (D165) |
...
> フレーム間の 6 ルールのうち 4 つが blocking で，生成した動きは実際に踏みうる．
>
> **そしてそこに穴があった** — 生成の輪は**フレーム間のルールを掛けていなかった**
> ので，`px gen prog --frames N` は**自分の検査に落ちる列を «通った» と言えた**．
> 直して試験で固定した (直す前は 1 回目で受理されることを確かめてある) ．

> [!note] **32x32 は 672 秒かかった** (16x16 の 80.4 秒に対し 8.4 倍) ．
> D162 の «無音の上限は 60 秒» とあわせると，**一括で受けていたら 32x32 は
> 手が届かない**．流で受ける形にしたことは «あれば良い» ではなく前提である．
