...
- 更新: 2026-08-14 (**M0 〜 M5 ・M7 完了．M6 は 3 件済 ・1 件は書かない．
  バックエンドを本物の API に当てて 2 件直した — 200 の応答はまだ未確認 (鍵は用意済み)．
  付録 C #5 を閉じ，残るは #1 と #4 の 2 件**)
- 仕様: Obsidian の `設計書/ドット絵CLI-Rust/` — `設計書.md` / `実装計画書.md` / `開発ノート.md`
- **`開発ノート.md` に「実装して初めて分かったこと」が全部入っている．** 設計書だけ読むと
  同じ穴に落ちる

> [!warning] **M6 は本物の API に当てたが，まだ «通って» はいない (2026-08-14)．**
> そのとき手元にあった鍵が**期限切れ**で `401 authentication_error` だった
> (鍵はその後差し替えられ，`~/.zshrc:162` のもので HTTP 200 を確認済み) ．
> **HTTP の往復は実測したが，200 の応答は 1 度も見ていない** — 送る本文が
> 受理されるか ・応答から L0 を取り出せるか ・何回で通るかは**まだ文書から
> 書いたまま**である．
>
> **叩いてすぐ 2 件出た** (どちらも直して試験を置いた．`docs/status.md` の M6 節)．
> **401 という «最も単純なエラー» でさえ道具は理由を言えなかった**ので，
> 同じ密度で誤りが残っていると考えること．
>
> **送る本文の側は仕様と突き合わせて確かめた** — `fallbacks: "default"` と
> beta ヘッダの対 ・`effort` の位置 ・温度と種を送らないこと ・`thinking` を
> 書かないこと ・`stop_reason` の扱いはすべて正しい (M6 節の表) ．
>
> **有効な鍵で下を 1 度通し，出た差分を `docs/status.md` の M6 節へ書く**:
>
> ```sh
> # ~/.zshrc:162 に有効な鍵がある (2026-08-14 に HTTP 200 を確認) ．
> # **非対話シェルには export が届かない** ($ANTHROPIC_API_KEY が空だった) ので明示的に渡す
...
### 0. **いま開いているもの (2026-08-14 時点)**

M0 〜 M5 ・M7 は閉じた．**M6 が 3 件済 ・1 件は書かない**で，残っているのは
下の 4 件である．**上から順に価値が高い．**

| # | やること | なぜ | 上限 |
| --- | --- | --- | --- |
| **1** | **`px gen prog` を 1 度通す (鍵は用意済み)** | 当てたが**200 の応答を 1 度も見ていない** | 読める (1 回叩けば分かる) |
| ~~2~~ | ~~付録 C #5 (`--method fast` が使える確率)~~ | **閉じた (D161)．47.5%** | — |
| 2 | 付録 C #1 (ラン長の谷の誤検出) | M3 から開いたまま | 読める |
| 3 | 付録 C #4 (局所格子推定の窓サイズ) | M2 から開いたまま | 読める |

#### 1. M6 の実測 — **最優先．半分だけ済んでいる**
...
**残っているのは下の 3 行だけ**である (1 行目は仕様と突き合わせて確かめた) ．
出た差分は `docs/status.md` の M6 節へ書く．

| 見るもの | 状態 |
| --- | --- |
| 送った本文が受け取られるか | **仕様と突き合わせて確かめた** — `fallbacks` と beta の対 ・`effort` の位置 ・温度と種 ・`thinking` ・`cache_control` はすべて正しい (status.md の表) ．ただし **200 で受理されたことは未確認** |
| 応答から L0 が取り出せるか | **未確認**．構造化出力の受け方 (`content` の `text` ブロック → `l0` 欄) |
| 何回で通るか | **未確認**．弱いなら**輪の回数ではなくプロンプトを直す** |
| `stop_reason` の扱い | `refusal` ・`max_tokens` は書いたが**実物を見ていない** |

**輪の側は偽バックエンドで試験してある**ので，落ちるとすれば
`crates/px-gen/src/anthropic.rs` の中だけである．
...
> **`--dry-run` は当てにならない** — 組み立てた依頼を見せず，宛先と寸法の行しか
> 出さない (前の引き継ぎはこれを «依頼を読め» と書いていたが，読めない) ．
> **本文を出せるようにするのが，鍵を使わずに送る側を確かめる唯一の道である．**

#### 2 ・3. 付録 C の残り — **2 件だけになった**

| # | 項目 | 状態 |
| --- | --- | --- |
| 1 | ラン長の谷検出の誤検出 (手描きの意図的な凹凸) | **開いている** (M3) |
| ~~2~~ | ~~形の乱雑さの閾値~~ | 閉じた (D143) |
| ~~3~~ | ~~«軌跡» の定式化~~ | 閉じた (D139) |
| 4 | 局所格子推定の窓サイズ | **開いている** (M2) |
| ~~5~~ | ~~`--method fast` が中間フレームとして使える確率~~ | **閉じた (D161)．47.5% (61 枚中 29 枚)** |
