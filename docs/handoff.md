...
| ブランチ | `main` |
| コミット | **D162 〜 D170 はすべてコミット済み．作業ツリーはきれい** (最後のコミットは `git log -1` で見る — 番号をここに書くと自分を追いかける) |
...
| # | やること | なぜ | 上限 |
| --- | --- | --- | --- |
| **1** | **M6 の «輪» の残り — `Refused` だけ** | 輪は回り助言も効いた (D166) ・`Truncated` も取った (D165) | 読める |
...
| `Refused` | **未実測**．偽バックエンドの試験のみ — **輪で残っているのはこれだけ** |
