...
| ブランチ | `main` |
| コミット | **`1928c74` まで**．D162 〜 D167 はすべてコミット済み (**作業ツリーはきれい**) |
...
| 2 | D163 が残した設計判断 (下) | 正しく描いた線が `px smooth` に壊される | **D32 に触る** |
...
#### 2. D163 が残した設計判断 — **決めてから触ること**

ジャギー検出は**正しく描いた傾きの直線で必ず鳴る** ($2(b \bmod a) \geq a$)．
`px smooth` は清書を 88 画素 ・17 / 58 枚 書き換える．直すには «谷を数える» の
手前で «走りが 2 種類しか無くその並びが傾きから説明できるなら幾何である» と
読む必要があり，**D32 (谷検出を採る) の設計判断に触る**．測っただけで止めてある．
