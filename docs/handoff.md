...
| ブランチ | `main` |
| コミット | **`bbc2e49` まで** (D162 ・D163 をコミット済み) ．**D164 (付録 C #4) は作業ツリー** |
...
| # | やること | なぜ | 上限 |
| --- | --- | --- | --- |
| **1** | **M6 の «輪» を実測する** | 1 発で通ったので**作り直しの輪は 1 度も回っていない** | 読める |
...
#### 1. M6 の «輪» — **通ったが，回っていない**

`px gen prog` は**1 回目で通った** (blocking 0 ・advisory 2 ・80.4 秒) ．
つまり**作り直しの輪は本物の応答では 1 度も回っていない**．

| 見るもの | 状態 |
| --- | --- |
| 送った本文が受理されるか | **済**．`output_config` ・`fallbacks` ・`cache_control` とも弾かれない |
| 応答から L0 が取り出せるか | **済** (流で受ける形に書き直した．D162) |
| 何回で通るか | **1 回**．2 回目以降は**未実測** |
| `Truncated` ・`Refused` | **未実測**．偽バックエンドの試験のみ |
