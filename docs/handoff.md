...
- 更新: 2026-08-15 (**M0 〜 M7 完了．M6 は本物の API で通った (D162)．
  付録 C #1 を閉じた (D163)．残るは #4 の 1 件だけ**)
...
| ブランチ | `main` |
| コミット | **`2165bec` まで**．**D162 (流で受ける) と D163 (付録 C #1) は未コミットで作業ツリーに載ったまま** — 先に `git diff` を読むこと |
...
M0 〜 M7 は閉じ，**M6 は本物の API で通った** (D162)．**付録 C も #4 だけ**になった．

| # | やること | なぜ | 上限 |
| --- | --- | --- | --- |
| **1** | **付録 C #4 (局所格子推定の窓サイズ)** | M2 から開いたまま．**付録 C で最後の 1 件** | 読める |
| 2 | **M6 の «輪» を実測する** | 1 発で通ったので**作り直しの輪は 1 度も回っていない** | 読める |
| 3 | D163 が残した設計判断 (下) | 正しく描いた線が `px smooth` に壊される | **D32 に触る** |
| ~~4~~ | ~~`px gen prog` を 1 度通す~~ | **通った (D162)** ・1 発 ・80.4 秒 | — |
| ~~5~~ | ~~付録 C #1 (ラン長の谷の誤検出)~~ | **閉じた (D163)** | — |
| ~~6~~ | ~~付録 C #5 (`--method fast`)~~ | **閉じた (D161)．47.5%** | — |

#### 1. 付録 C #4 — 局所格子推定の窓サイズ (M2)

`docs/investigations/grid-calibration.md` に M2 の校正記録がある．**推定器は
マクロ平均 88.1% で実装計画書の目標 95% に未達のまま M2 を閉じている**ので，
#4 はその続きである．**まず grid-calibration.md の «いま残っていること» を読む．**

#### 2. M6 の «輪» — **通ったが，回っていない**
